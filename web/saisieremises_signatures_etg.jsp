...
<html>
    <head>
        ...
        <title>Saisie Remises Signatures</title>
        <style type="text/css">
        </style>
    ...
    <body onload="setFocus()">
        <%
            ...
            if (params_auto == null || (params_auto != null && params_auto.equalsIgnoreCase("1"))) {
                if (action != null && (action.equalsIgnoreCase("precedent") || action.equalsIgnoreCase("suivant"))) {
                    ...
                    String sql = "SELECT * FROM SEQUENCES WHERE (MACHINESCAN IN (SELECT MACHINE FROM MACUTI WHERE UTILISATEUR = '" + user.getLogin().trim() + "')) "
                            + "  AND (UTILISATEUR='' OR UTILISATEUR IS NULL OR UTILISATEUR = '" + user.getLogin().trim() + "') ORDER BY MACHINESCAN,IDSEQUENCE FOR UPDATE";
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><input type="text" name="numero" value="" maxlength="12" onKeypress="return numeriqueValide(event);" onblur=" chargeInfoCompteECI();" /></td>
                                    <% } else {%>
                                <td><input type="text" name="numero" value="${requestScope['compteRemettant']}" maxlength="12" onKeypress="return numeriqueValide(event);" onblur=" chargeInfoCompteECI();" /></td>
                                    <%}
                                        ;%>
                                <td></td>
                                <td></td>
                            ...
                            <tr>
                                <td>Agence: </td>
                                <td> 
                                    <a:widget name="dojo.combobox" id="agenceCompte"  
                                              value="" subscribe="/cl" publish="/choix/agenceECI"/>	
        ...
        <div id="tableSaisieChequeDIV2">
            <fieldset id="divField">
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td></td>
                                <td>Remise: </td>
                                <td><input type="button" name="idremise" value="${requestScope['idremise']}" onclick="location.replace('saisieremises_signatures_etg.jsp?action=remise&idremise=${requestScope["idremise"]}')"/></td>
                ...
                <b><center>Cheque ${requestScope['index']} sur ${requestScope['nbOpers']}</center></b>
                <br>
                <%if (index > 1) {%>
                <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('saisieremises_signatures_etg.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
                <%}%>
                <%if (nbOpers > index) {%>
                <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('saisieremises_signatures_etg.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
                <%}%>
            </fieldset>
        </div>
        <%}%>
