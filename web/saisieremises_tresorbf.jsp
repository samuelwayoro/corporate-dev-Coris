<%@page contentType="text/html" import="org.patware.jdbc.DataBase,java.math.BigDecimal,clearing.table.Utilisateurs,clearing.web.bean.ComboBordereauBean"%>
<%@page pageEncoding="UTF-8" import="org.patware.xml.JDBCXmlReader,org.patware.web.bean.MessageBean,org.patware.utils.Utility"%>
<%@page import="org.patware.xml.JDBCXmlReader,clearing.table.Sequences,clearing.table.Remises,clearing.table.Sequences,clearing.table.Cheques"%>
...
<html>
    ...
    <body onload="setFocus()">
        ...
        ComboBordereauBean comboBordereauBean = new ComboBordereauBean(user);
        session.setAttribute("comboBordereauBean", comboBordereauBean);
    ...
    if (message != null && message.getMessage() != null) {
        ...
        <div id="tableSaisieRemiseDIV" align="center">
            ...
            <form  id="remiseForm" name="remiseForm" onsubmit=" return false;">
                <table>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td><a:widget name="dojo.combobox" id="bordereau"  value="${comboBordereauBean.comboDatas}"  publish="/cl/getBordereau"/></td>
                            
                            <td></td>
                            <td></td>
                            <td>Montant: </td>
                          
                                <td><input type="text" name="montantRemise" value="" onchange="return(currencyFormat(this,' ',event));" disabled/></td>
                ...
                <input type="hidden" value="checkRemiseTresor" name="action" />
                <input type="hidden" value="" name="noex" />
                <input type="hidden" value="" name="idop" />
                <input type="hidden" value="" name="cdscpta" />
                <input type="hidden" value="" name="typeinter" />
                <input type="hidden" value="00" name="escompte" />
                <input type="hidden" value="" name="reference" />
                <input type="hidden" value="01001" name="agence" />
                <input type="hidden" value="011111111111" name="numero" />
...
<fieldset id="divField">
            ...
            <form  id="chequeForm" name="chequeForm" onsubmit="return false;">
                <table>
                    ...
                    <tbody>
                        ...
                        <%};%>
                            ...
                            <td></td>
                            <td>Remise: </td>
                            <td><input type="button" name="idremise" value="${requestScope['idremise']}" onclick="location.replace('saisieremises_tresorbf.jsp?action=remise&idremise=${requestScope["idremise"]}')"/></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>Emetteur: </td>
                            <td><input type="text" name="nomTire" value="${requestScope['nomTire']}" maxlength="25" tabindex="2"/></td>
                            <td></td>
                            <td></td>
                            <td>Date Emission:</td>
                            <td tabindex="3"><a:widget name="dojo.dropdowndatepicker" id="dateemission" /> </td>
                            <td>
                            </td>
                        </tr>
                        <tr>
                            <td>Banque: </td>
                            <td><input type="text" name="codeBanque" value="${requestScope['codeBanque']}" maxlength="5" tabindex="4"/></td>
                            <td></td>
                            <td></td>
                            <td>Agence:</td>
                            <td><input type="text" name="codeAgence" value="${requestScope['codeAgence']}" maxlength="5" onKeypress="return numeriqueValide(event);" tabindex="5"/></td>
                            <td>
                            </td>
                        </tr>
                        <tr>
                            <td>Numero de Compte: </td>
                            <td><input type="text" name="numeroCompte"  value="${requestScope['numeroCompte']}" maxlength="12" onKeypress="return numeriqueValide(event); " tabindex="6"/></td>
                            <td><input type="text" name="clerib" size="2" value="${requestScope['clerib']}"  maxlength="2" tabindex="7"/></td>
                            <td></td>

                            <td>Numero de Cheque:</td>
                            <td><input type="text" name="numeroCheque"  value="${requestScope['numeroCheque']}" maxlength="7" onKeypress="return numeriqueValide(event);" tabindex="8"/></td>
                            <td></td>
                            <td><input type="submit" name="valider" value="Valider" onclick="checkCheque();" tabindex="9"/></td>
                ...
                <input type="hidden" value="checkChequeTresor" name="action" />
                <input type="hidden" value="${requestScope['index']}" name="index" />
            </form>
            <b>Cheque ${requestScope['index']} sur ${requestScope['nbOpers']}</b>
            <br>
            <%if (index > 1) {%>
            <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('saisieremises_tresorbf.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
            <%if (nbOpers > index) {%>
            <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('saisieremises_tresorbf.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
            </fieldset>
        </div>
        <%}%>
