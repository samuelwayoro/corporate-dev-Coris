...
<body onload="affiche_master();" >
    <%
        ...
        comboUtilisateurBean.setRequete("SELECT * FROM UTILISATEURS WHERE ADRESSE='" + user.getAdresse().trim() + "' AND TRIM(LOGIN) IN (SELECT DISTINCT TRIM(NOMUTILISATEUR) FROM REMISES WHERE ETAT IN (" + Utility.getParam("CETAOPEVALITE") + "," + Utility.getParam("CETAOPEVALITE2") + " ))");
    ...
    <%
        ...
        String query = "select C1.IDCHEQUE,C1.REMISE,C1.DEVISE,C1.NUMEROCHEQUE,C1.NUMEROCOMPTE,C1.NOMBENEFICIAIRE,C1.ETAT,C1.AGENCE,C1.COMPTEREMETTANT,C1.AGENCEREMETTANT,C1.DATETRAITEMENT,C1.DATECOMPENSATION,"
                ...
                + "and C1.etat in (30,40)  "
                + "and C1.datetraitement ='" + dateTraitement + "' and C1.remise=R.idremise "
                + "and R.etablissement like '" + user.getAdresse().trim() + "' ";
...
//VALIDATION CHEZ LE CORPORATES REMISE PAR REMISE
        boolean finish = false;
        String sql = "SELECT * FROM REMISES WHERE IDREMISE = (SELECT MIN(IDREMISE) FROM REMISES WHERE ETAT=" + Utility.getParam("CETAOPEVALITE")
        ...
        if (remises != null && remises.length > 0) {
            ...
            if (cheques != null && cheques.length > 0) {
                fPictureCheque = cheques[0].getPathimage().substring(3) + "\\" + cheques[0].getFichierimage() + "f.jpg";
                rPictureCheque = cheques[0].getPathimage().substring(3) + "\\" + cheques[0].getFichierimage() + "r.jpg";
            }
            ...
            sql = "SELECT IDCHEQUE as \"IDCHEQUE\",NUMEROCHEQUE, MONTANTCHEQUE, BANQUE,AGENCE,NUMEROCOMPTE, GARDE,CALCUL,DATEECHEANCE,PATHIMAGE,FICHIERIMAGE FROM CHEQUES "
    ...
    <form name="detailForm" id="detailForm">
        <div id="monitorPanelDIV">
            <input type='button' name='valider' value='Valider' onclick=" validerRemiseBatchLite();"/>
            <input type='button' name='rejeter' value='Rejeter' onclick=" rejeterRemiseBatchLite();"/>
        </div>
