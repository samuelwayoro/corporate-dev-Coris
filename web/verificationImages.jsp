...
<body>
             <%
              ...
              comboBanqueBean.setRequete("select * from banques where codebanque in (select distinct banqueremettant from cheques where etat in ("+ Utility.getParam("CETAOPERETREC")+")  AND (LOTSIB IS NULL OR LOTSIB=2) )");
            ...
            <form name='choixCodeBanques'>
              ...
              <input type="button" name="startVal" value="Demarrer la Vérification des Images" onclick="startValidationImages();">
                <input type="hidden" name="codeBanque" value=""/>
                <input type="hidden" name="action" value="startValidationImages"/>
            </form>
        </div>
              <%}else{
              ...
              int nbChqRestant = 0;
              String sql = "SELECT * FROM CHEQUES WHERE ETAT="+ Utility.getParam("CETAOPERETREC")+
                           " AND (LOTSIB IS NULL OR LOTSIB=2) "+
                           " AND BANQUEREMETTANT LIKE ('"+ banqueChoisie+"')"+
                           " AND DATECOMPENSATION >=( SELECT MAX(DATECOMPENSATION) FROM CHEQUES  WHERE ETAT="+ Utility.getParam("CETAOPERETREC")+")"+
                           " AND (CODEUTILISATEUR='' OR CODEUTILISATEUR IS NULL OR CODEUTILISATEUR = '" + user.getLogin().trim() + "') ORDER BY LOTSIB desc, IDCHEQUE";
 ...
 <%if (!finish) {%>
    ...
    <form name="detailForm" id="detailForm">
            <input name="printMaster" type="button"  onClick="printSignDiv('imageDIV2');" value="<-Imprimer">
             ...
             <input type='button' name='valider' value='Valider' onclick=" validerImageCheque();"/>
             <input type="hidden" name="idObjet" value="${requestScope['idObjet']}"/>
             <input type='hidden' name='etatdebut' value='(152)' />
             <input type='hidden' name='etatfin' value='250' />
             <input type="hidden" name="action" value="validerImageCheque"/>
        </form>
