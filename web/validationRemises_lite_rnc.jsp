...
<html>
    ...
    <body>
        <%//VALIDATION A LA BANQUE CHEQUE PAR CHEQUE DU CORPORATES
            ...
            comboEtablissementsBean.setRequete("SELECT * FROM ETABLISSEMENTS WHERE CODEETABLISSEMENT "
                    + " IN (SELECT DISTINCT TRIM(ETABLISSEMENT) FROM REMISES WHERE ETAT IN (" + Utility.getParam("CETAOPEVALDELTA") + "," + Utility.getParam("CETAOPEVALDELTA2") + " ))");
        ...
        <%} else {
            ...
            if (action != null && (action.equalsIgnoreCase("precedent") || action.equalsIgnoreCase("suivant"))) {
                ...
                String sql = "SELECT r.* FROM REMISES r, CHEQUES c WHERE r.idremise=c.remise and r.ETAT IN (" + Utility.getParam("CETAOPEVALDELTA") + "," + Utility.getParam("CETAOPEVALDELTA2")
                        + ") AND TRIM(r.ETABLISSEMENT) LIKE '" + etablissementDepot.trim() + "'"
                        + " AND c.banque=c.banqueremettant "
                        + " ORDER BY r.ETAT DESC, r.IDREMISE ASC";
                Remises[] remises = (Remises[]) db.retrieveRowAsObject(sql, new Remises());
                BigDecimal lastIdSeq = null, lastId = null;
                if (remises != null && remises.length > 0) {
                    if (remises[0].getEtat().equals(new BigDecimal(Utility.getParam("CETAOPEVALDELTA")))) {
                        lastDocType = "REMISE";
                        ...
                        sql = "SELECT * FROM CHEQUES WHERE IDCHEQUE = (SELECT MIN(IDCHEQUE) FROM CHEQUES WHERE REMISE=" + remises[0].getIdremise() + " AND ETAT IN (" + Utility.getParam("CETAOPEVALDELTA") + "))";
                        Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                        if (cheques != null && cheques.length > 0) {
                            lastDocType = "CHEQUE";
                    ...
                    if (lastDocType.equalsIgnoreCase("REMISE")) {
                        ...
                        sql = "SELECT * FROM CHEQUES WHERE IDCHEQUE=" + lastId + " AND ETAT =" + Utility.getParam("CETAOPEVALDELTA") + " AND TRIM(ETABLISSEMENT) LIKE '" + etablissementDepot.trim() + "' AND SEQUENCE=" + lastIdSeq;
                        Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                        if (cheques != null && cheques.length > 0) {
                    ...
                    if (Utility.getParam("VALIDATION_CHEQUE_AUTO") != null && Utility.getParam("VALIDATION_CHEQUE_AUTO").equalsIgnoreCase("AUTO")) {
                        ...
                        etat = Utility.getParam("CETAOPESUPVALSURCAI");
                    }
    ...
    <div align="center">
        ...
        <form name='rejetForm'>
            <div align="center">
                ...
                <input type='hidden' name='action' value='rejectDocBtnRNC' />
            </div>
        </form>
    ...
    <div id="tableSaisieRemiseDIV" align="center">
        ...
        <form  id="remiseForm" name="remiseForm" onsubmit=" return false;">
            ...
            <input type="hidden" value="valideRemiseLiteRNC" name="action" />
    ...
    <%}
      ...
      <div id="imageDIV4">
        ...
        <form  id="chequeForm" name="chequeForm" onsubmit="return false;">
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td></td>
                        <td>Remise: </td>
                        <td><input type="button" name="idremise" value="${requestScope['idremise']}" onclick="location.replace('validationRemises_lite_rnc.jsp?action=remise&idremise=${requestScope["idremise"]}')"/></td>
                        <td></td>
                    </tr>
            ...
            <input type="hidden" value="valideChequeLiteRNC" name="action" />
            <input type="hidden" value="${requestScope['index']}" name="index" />
        </form>
        <b>Cheque ${requestScope['index']} sur ${requestScope['nbOpers']}</b>
        <br>
        <%if (index > 1) {%>
        <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('validationRemises_lite_rnc.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
        <%if (nbOpers > index) {%>
        <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('validationRemises_lite_rnc.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
    </div>
    <%}
        }%>
