...
<html>
    ...
    <body onload="setFocus()">
        <%
            ...
            if (params_auto == null || (params_auto != null && params_auto.equalsIgnoreCase("1"))) {
                ...
                if (action != null && (action.equalsIgnoreCase("precedent") || action.equalsIgnoreCase("suivant"))) {
                    ...
                    String sql = "SELECT * FROM SEQUENCES WHERE (MACHINESCAN IN (SELECT MACHINE FROM MACUTI WHERE UTILISATEUR = '" + user.getLogin().trim() + "')) AND (UTILISATEUR='' OR UTILISATEUR IS NULL OR UTILISATEUR = '" + user.getLogin().trim() + "') ORDER BY IDSEQUENCE,MACHINESCAN FOR UPDATE";
                    ...
                    if (sequences != null && sequences.length > 0) {
                        ...
                        request.setAttribute("dateScan", sequences[0].getDatedescan());
        ...
        <div align="center" >
            ...
            <font style="font-weight:bold;color:blue"> | Date de Scan: ${requestScope['dateScan']}</font>
        </div>
        ...
        <div id="tableSaisieChequeDIV2">
            <fieldset id="divField">
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td></td>
                                <td>Remise: </td>
                                <td><input type="button" name="idremise" value="${requestScope['idremise']}" onclick="location.replace('saisieremises_signatures_cbao.jsp?action=remise&idremise=${requestScope["idremise"]}')"/></td>
                            ...
                            <tr>
                                ...
                                <td></td>
                                <td>Agence:</td>
                                <td><input id="codeAgence" type="text" name="codeAgence" value="${requestScope['codeAgence']}" maxlength="5" onblur=" refreshSignaturePanel();"
                                           onKeypress="return numeriqueValide(event);" tabindex="3"/></td>
                                <td>
                                </td>
                            </tr>
                            <tr>
                                <td>Numero de Compte: </td>
                                <td><input type="text" id="numeroCompte" name="numeroCompte"  value="${requestScope['numeroCompte']}" maxlength="12" onblur=" refreshSignaturePanel();"
                                           onKeypress="return numeriqueValide(event);" tabindex="4"/></td>
                                <td><input type="text" name="clerib" size="2" value="${requestScope['clerib']}"  maxlength="2" tabindex="5"/></td>
                                <td><input type="button" name="signature" value="S" onClick="refreshSignature();" </td>
                ...
                <br>
                <%if (index > 1) {%>
                <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('saisieremises_signatures_cbao.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
                <%}%>
                <%if (nbOpers > index) {%>
                <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('saisieremises_signatures_cbao.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
                <%}%>
            </fieldset>
        </div>
        <%}%>
