...
<html>
   ...
   <jsp:useBean id="comboUtilisateurBean" class="clearing.web.bean.ComboUtilisateurBean" />
    ...
    <body>
        <%
            //VALIDATION CHEZ LE CORPORATES CHEQUE PAR CHEQUE
        ...
        comboUtilisateurBean.setRequete("SELECT * FROM UTILISATEURS WHERE ADRESSE='" + user.getAdresse().trim() + "' "
                + "AND TRIM(LOGIN) IN (SELECT DISTINCT TRIM(NOMUTILISATEUR) FROM REMISES WHERE ETAT IN (" + Utility.getParam("CETAOPEVALITE") + ","+ Utility.getParam("CETAOPEVALITE2")+" ))");
    ...
    <div align="center">
        ...
        <%} else {
            ...
            if (action != null && (action.equalsIgnoreCase("precedent") || action.equalsIgnoreCase("suivant"))) {
                ...
                String sql = "SELECT * FROM REMISES WHERE ETAT IN (" + Utility.getParam("CETAOPEVALITE") + "," + Utility.getParam("CETAOPEVALITE2")
                        + ")  AND NOMUTILISATEUR LIKE '" + userSaisie.trim() + "'" 
                        + " ORDER BY ETAT DESC, IDREMISE ASC";
                Remises[] remises = (Remises[]) db.retrieveRowAsObject(sql, new Remises());
                BigDecimal lastIdSeq = null, lastId = null;
                if (remises != null && remises.length > 0) {
                    if (remises[0].getEtat().equals(new BigDecimal(Utility.getParam("CETAOPEVALITE")))) {
                        lastDocType = "REMISE";
                        ...
                        sql = "SELECT * FROM CHEQUES WHERE IDCHEQUE = (SELECT MIN(IDCHEQUE) FROM CHEQUES WHERE REMISE=" + remises[0].getIdremise() + " AND ETAT IN (" + Utility.getParam("CETAOPEVALITE") + "))";
                        Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                        if (cheques != null && cheques.length > 0) {
                            lastDocType = "CHEQUE";
                    ...
                    if (lastDocType.equalsIgnoreCase("REMISE")) {
                        ...
                        sql = "SELECT * FROM CHEQUES WHERE IDCHEQUE=" + lastId + " AND ETAT =" + Utility.getParam("CETAOPEVALITE") + " AND CODEUTILISATEUR LIKE '" + userSaisie.trim() + "' AND SEQUENCE=" + lastIdSeq;
                        Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                        if (cheques != null && cheques.length > 0) {
                    ...
                    //Ajout du texte CUT OFF Heure du cut off ==CUTOFF_HOUR                                       
                   ...
                   String cutoffHour = Utility.getParam("CUTOFF_HOUR");
                    ...
                    String interval = "Utilisateur de Saisie: " + userSaisie.trim() + "| Date de Traitement du " + dateTraitement;
                    ...
                    String query = "select C1.IDCHEQUE,C1.REMISE,C1.DEVISE,C1.NUMEROCHEQUE,C1.NUMEROCOMPTE,C1.NOMBENEFICIAIRE,C1.ETAT,C1.AGENCE,C1.COMPTEREMETTANT,C1.AGENCEREMETTANT,C1.DATETRAITEMENT,C1.DATECOMPENSATION,"
                            ...
                            + "and R.nomutilisateur like '" + userSaisie.trim() + "' ";
        %>
    ...
    <div align="center">
        ...
        <form name='rejetForm'>
            <div align="center">
                ...
                <input type='hidden' name='action' value='rejectDocCorpBtn' />
            </div>
        </form>
    ...
    <div id="tableSaisieRemiseDIV" align="center">
        ...
        <form  id="remiseForm" name="remiseForm" onsubmit=" return false;">
            ...
            <input type="hidden" value="valideRemiseLiteCorporates" name="action" />
    ...
    <div id="tableSaisieChequeDIV" align="center">
        ...
        <form  id="chequeForm" name="chequeForm" onsubmit="return false;">
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td></td>
                        <td>Remise: </td>
                        <td><input type="button" name="idremise" value="${requestScope['idremise']}" onclick="location.replace('validationRemises_lite_corporates.jsp?action=remise&idremise=${requestScope["idremise"]}')"/></td>
                        <td></td>
                    </tr>
            ...
            <input type="hidden" value="valideChequeLiteCorporate" name="action" />  
            <input type="hidden" value="${requestScope['index']}" name="index" />
        </form>
        <b>Cheque ${requestScope['index']} sur ${requestScope['nbOpers']}</b>
        <br>
        <%if (index > 1) {%>
        <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('validationRemises_lite_corporates.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
        <%if (nbOpers > index) {%>
        <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('validationRemises_lite_corporates.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
    </div>
    <%}
            }%>
