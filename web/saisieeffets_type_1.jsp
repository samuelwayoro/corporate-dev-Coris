                ...
                if (action != null && (("precedent|remise").contains(action) || action.equalsIgnoreCase("suivant"))) {
                    ...
                    if (sequences != null && sequences.length > 0) {
                        ...
                        String query = "select C1.IDEFFET,C1.REMISE,C1.REFREMISE,C1.DEVISE,C1.NUMEROEFFET,C1.NUMEROCOMPTE_TIRE,C1.NOM_BENEFICIAIRE,C1.ETAT,C1.AGENCE,C1.numerocompte_beneficiaire,C1.AGENCEREMETTANT,C1.DATETRAITEMENT,C1.DATECOMPENSATION,"
                                ...
                                + "from effets C1,effets C2, Banques B1,Banques B2,Remises R "
...
<!--                            <td>
                                <SELECT name="typeEffet" id="typeEffet" size="1"  >
                                    <OPTION value="${requestScope['typeEffet']}" selected></OPTION>
                                    <OPTION value="045">045-Billet à ordre  </OPTION>
                                    <OPTION value="046">046-Lettre de change</OPTION>
                                </SELECT>
                            </td>-->
...
<!--                            <td><input type="checkbox" id="force" name="forcage" value="OFF" />Forcage Doublon</td>-->
            ...
            <br>
            <%if (index > 1) {%>
            <input type="button" value="Remise" name="Remise" onclick="window.location.replace('saisieeffets_type.jsp?action=remiseseffets&ideffet=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('saisieeffets_type.jsp?action=precedent&ideffet=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
            <%if (nbOpers > index) {%>
            <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('saisieeffets_type.jsp?action=suivant&ideffet=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
        </fieldset>
    </div>
