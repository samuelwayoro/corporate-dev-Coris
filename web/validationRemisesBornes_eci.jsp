...
<body onload="affiche_master();" >
    <%
        ...
        String sql = "SELECT * FROM REMISES WHERE IDREMISE = (SELECT MIN(IDREMISE) FROM REMISES WHERE ETAT=" + Utility.getParam("CETAREMSAIIRIS")
                ...
                + " AND LOTAGENCE= " + Utility.getParam("CETAREMSIGVER")
                ...
                + " UNION SELECT * FROM REMISES WHERE IDREMISE = (SELECT MIN(IDREMISE) FROM REMISES WHERE ETAT=" + Utility.getParam("CETAREMSAIIRIS")
                + " AND AGENCEDEPOT='" + user.getAdresse().trim() + "'"
                + " AND IDREMISE IN (SELECT REMISE FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAREMSAIIRIS") + " ) AND BANQUE<>BANQUEREMETTANT AND CODEREPRESENTATION IS NOT NULL)"
                + " AND (VALIDEUR='' OR VALIDEUR IS NULL OR VALIDEUR = '" + user.getLogin().trim() + "')) ";
 ...
 var slideBaniere = new Fx.Slide('imageDetailDIV');
   ...
   <script>
    ...
    var slideMaster =new Fx.Slide('imageMasterDIV');
        ...
        $('p        rive_btn_masque_master').addEvent('click', function(e){
                e = new Event(e);
        slideMaster.toggle();
        e.stop();
