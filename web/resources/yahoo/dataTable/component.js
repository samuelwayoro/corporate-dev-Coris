...
jmaki.widgets.yahoo.dataTable.Widget = function (wargs) {
    ...
    var rowsPerPage = 10;
    var pageLinks = 10;
    var dropdownOptions = [5, 10, 50, 500];
    ...
    if (wargs.args) {
        ...
        if (typeof args.scrollable != 'undefined') {
            // set scrollable var
            scrollable = args.scrollable;
        }
        if (typeof args.pageLinks != 'undefined') {
            pageLinks = args.pageLinks;
        }
        if (typeof args.dropdownOptions != 'undefined') {
    ...
    this.addRows = function (_d, index) {
        ...
        for (var i = 0; i < _rows.length; i++) {
            ...
            if (idMappings[_lid]) {
                ...
                while (idMappings[_lid])
                    _lid = genId();
            }
            idMappings[_lid] = rec;
        }
        // _widget.dataTable.sortColumn(_widget.columns.keys[0], YAHOO.widget.DataTable.CLASS_ASC);
        _widget.dataTable.refreshView();
        if (_widget.paginator) {
            _widget.paginator.setTotalRecords(_widget.dataTable.getRecordSet().getLength(), true);
            _widget.paginator.render();
        }
    }
    ...
    this.getValue = function () {
        // iterate through the resoruce set and match up the jMaki ids
        // or do the matching early.
        var model = {};
        model.columns = jmaki.clone(cold);
        var records = _widget.dataTable.getRecordSet().getRecords();
        model.rows = [];
        for (var i = 0; i < records.length; i++) {
            model.rows.push(jmaki.clone(records[i]._oData));
        }
        return model;
    };
    ...
    function init() {
        ...
        for (var i = 0; i < cold.length; i++) {
            var col = {};
            var sortable = true;
            if (cold[i].sortable == false)
                sortable = cold[i].sortable;
        ...
        var onCellEdit = function (oArgs) {
            jmaki.publish(publish + "/onCellEdit", {
                widgetId: wargs.uuid,
                topic: publish,
                type: 'onCellEdit',
                oldValue: oArgs.oldData,
                newData: oArgs.newData
            });

            alert("Cell \"" + oArgs.target.id +
                    "\" was updated from \"" + oArgs.oldData + "\" to \"" +
                    oArgs.newData + "\"", "info", this.toString());

            jmaki.publish(publish + "/onSave", {
                widgetId: wargs.uuid
            });
        }
        ...
        if (paginated) {
            ...
            var _pageOptions = {
                ...
                template: "{FirstPageLink} {PreviousPageLink}{PageLinks}{NextPageLink} {LastPageLink}  AFFICHER {RowsPerPageDropdown} LIGNES PAR PAGE",
            ...
            if (wargs.args && wargs.args.paginatorOptions) {
                _pageOptions = wargs.args.paginatorOptions;
            }
            options.paginator = new YAHOO.widget.Paginator(_pageOptions);
            _widget.paginator = options.paginator;
        }

        _widget.dataTable = new YAHOO.widget.DataTable(wargs.uuid, _widget.columns, ds, options);
        ...
        _widget.dataTable.subscribe("buttonClickEvent", function (args) {
            if (confirm('Etes-vous sur ?')) {


                var oRecord = this.getRecord(args.target);
                //Retrouver la clé primaire
                var cols = _widget.dataTable.getColumnSet().keys;
                for (var i = 0; i < cols.length; i++) {
                    if (cols[i].primaryKey) {
                        //  var message = cols[i].identifiant + "=" + oRecord.getData(cols[i].key);
                        //                    alert(args.target.value+''+args.target.name);
                        //Appeler le controller par Ajax
                        console.log("ControlServlet?action=" + args.target.name + "&primaryKey=" + cols[i].identifiant + "&primaryValue=" + oRecord.getData(cols[i].key));
                        jmaki.doAjax({
                            method: "POST",
                            url: "ControlServlet?action=" + args.target.name + "&primaryKey=" + cols[i].identifiant + "&primaryValue=" + oRecord.getData(cols[i].key),
                            callback: function (_req) {

                                var tmp = _req.responseText;

                                if (tmp == "rien") {
                                    alert("Erreur lors de la mise a jour, veuillez rafraichir!");
                                } else if (tmp == "OK") {
        ...
        _widget.dataTable.subscribe("editorSaveEvent", function (args) {
            //var idobjet = args.editor.getRecord().getData('1');
            ...
            for (var i = 0; i < cols.length; i++) {
                if (cols[i].primaryKey) {
                    if (cols[i].identifiant != args.editor.getColumn().identifiant) {
                        if (clause != '') {
                            clause += ' AND ';
                        }

                        clause += '' + cols[i].identifiant + "='" + args.editor.getRecord().getData(cols[i].key) + "'";
            ...
            var message = args.oldData;
            if (message == null) {
                message = "UPDATE " + args.editor.getColumn().table +
                        " SET " + args.editor.getColumn().identifiant + " ='" + args.newData +
                        "' WHERE " + clause +
                        " AND " + args.editor.getColumn().identifiant + " is " + args.oldData + "";
            } else {
                message = "UPDATE " + args.editor.getColumn().table +
                        " SET " + args.editor.getColumn().identifiant + " ='" + args.newData +
                        "' WHERE " + clause +
                        " AND " + args.editor.getColumn().identifiant + " ='" + args.oldData + "'";
            }

            //alert(args.oldData);
            jmaki.doAjax({
                method: "POST",
                url: "ControlServlet?action=updateDataTable&message=" + encodeURIComponent(message) +
                        "&table=" + args.editor.getColumn().table + "&colonne=" + args.editor.getColumn().identifiant +
                        "&oldvalue=" + args.oldData + "&newvalue=" + args.newData + "&primaryClause=" + clause,
                callback: function (_req) {
            ...
            //
            //
            //	        var pub = { widgetId : wargs.uuid, columnId : args.editor.column.key, columnIndex : args.editor.column._nKeyIndex, record : args.editor.record, cellValue : args.editor.record._oData[args.editor.column.key], rowId : getRecordMapping(args.editor.record._sId)};
            //	        jmaki.publish( publish + "/onCellEdit", pub);
            //            jmaki.publish(publish + "/onSave", { widgetId : wargs.uuid});
        });
        _widget.dataTable.subscribe("editorUpdateEvent", function (args) {
            // alert("test2");
            var pub = {
                widgetId: wargs.uuid,
                columnId: args.editor.column.key,
                columnIndex: args.editor.column._nKeyIndex,
                record: args.editor.record,
                value: args.editor.value,
                rowId: getRecordMapping(args.editor.record._sId)
            };
            jmaki.publish(publish + "/onCellUpdate", pub);
            // autosave if a radio button - fires save event
            if (args.editor.column.editor === "radio") {
                this.saveCellEditor();
            }
        });
        //  _widget.dataTable.subscribe("editorBlurEvent", _widget.dataTable.onEventSaveCellEditor);
        _widget.dataTable.subscribe("cellDblclickEvent", _widget.dataTable.onEventShowCellEditor);
        ...
        if (rowSingleSelect == true)
            _widget.dataTable.subscribe("cellClickEvent", _widget.dataTable.onEventSelectRow);
        if (rowSingleSelect == true)
            _widget.dataTable.subscribe("cellClickEvent", function (e) {
