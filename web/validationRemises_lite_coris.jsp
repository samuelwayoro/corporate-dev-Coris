...
<html>
    ...
    <body>
        <%//VALIDATION A LA BANQUE CHEQUE PAR CHEQUE DU CORPORATES
            DataBase db = new DataBase(JDBCXmlReader.getDriver());
            Utilisateurs user = (Utilisateurs) session.getAttribute("utilisateur");

            String etablissementDepot = (String) session.getAttribute("etablissementDepot");
            if (etablissementDepot == null) {
                etablissementDepot = "%";
            }
            db.open(JDBCXmlReader.getUrl(), JDBCXmlReader.getUser(), JDBCXmlReader.getPassword());
            String action = request.getParameter("choixEtablissement");
            comboEtablissementsBean.setRequete("SELECT * FROM ETABLISSEMENTS WHERE CODEETABLISSEMENT "
                    + " IN (SELECT DISTINCT TRIM(ETABLISSEMENT) FROM REMISES WHERE ETAT IN (" + Utility.getParam("CETAOPEVAL") + "," + Utility.getParam("CETAOPEVAL2") + " ))");

            if (action != null && action.equalsIgnoreCase("true")) {%>

        <div align="center">
            <form name='choixEtablissement'>
                Choisissez un etablissement : <a:widget name="dojo.combobox" id="idetablissement" value="${comboEtablissementsBean.comboDatas2}" />
                <input type="button" name="startVal" value="Demarrer la Validation des Remises" onclick="startValidationEtablissement();">
                <input type="hidden" name="etablissementChoisi" value=""/>
                <input type="hidden" name="action" value="choixEtablissement"/>
            </form>
        </div>
        <%} else {
            ...
            if (action != null && (action.equalsIgnoreCase("precedent") || action.equalsIgnoreCase("suivant"))) {
                ...
                String sql = "SELECT * FROM REMISES WHERE ETAT IN (" + Utility.getParam("CETAOPEVAL") + "," + Utility.getParam("CETAOPEVAL2")
                        + ") AND TRIM(ETABLISSEMENT) LIKE '" + etablissementDepot.trim()+"'"
                        + " ORDER BY ETAT DESC, IDREMISE ASC";
                Remises[] remises = (Remises[]) db.retrieveRowAsObject(sql, new Remises());
                BigDecimal lastIdSeq = null, lastId = null;
                if (remises != null && remises.length > 0) {
                    if (remises[0].getEtat().equals(new BigDecimal(Utility.getParam("CETAOPEVAL")))) {
                        lastDocType = "REMISE";
                        ...
                        sql = "SELECT * FROM CHEQUES WHERE IDCHEQUE = (SELECT MIN(IDCHEQUE) FROM CHEQUES WHERE REMISE=" + remises[0].getIdremise() + " AND ETAT IN (" + Utility.getParam("CETAOPEVAL") + "))";
                        Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                        if (cheques != null && cheques.length > 0) {
                            lastDocType = "CHEQUE";
                    ...
                    if (lastDocType.equalsIgnoreCase("REMISE")) {
                        ...
                        sql = "SELECT * FROM CHEQUES WHERE IDCHEQUE=" + lastId + " AND ETAT =" + Utility.getParam("CETAOPEVAL") + " AND TRIM(ETABLISSEMENT) LIKE '" + etablissementDepot.trim() + "' AND SEQUENCE=" + lastIdSeq;
                        Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                        if (cheques != null && cheques.length > 0) {
                    ...
                    if (Utility.getParam("VALIDATION_CHEQUE_AUTO") != null && Utility.getParam("VALIDATION_CHEQUE_AUTO").equalsIgnoreCase("AUTO")) {
                        ...
                        etat = Utility.getParam("CETAOPEVALDELTA");
                    }
                    ...
                    String interval = "Etablissement de Saisie: " + etablissementDepot.trim() + "| Date de Traitement du " + dateTraitement;
                    out.println(interval);
                    String query = "select C1.IDCHEQUE,C1.REFREMISE,C1.REMISE,C1.DEVISE,C1.NUMEROCHEQUE,C1.NUMEROCOMPTE,C1.NOMBENEFICIAIRE,C1.ETAT,C1.AGENCE,C1.COMPTEREMETTANT,C1.AGENCEREMETTANT,C1.DATETRAITEMENT,C1.DATECOMPENSATION,"
                            ...
                            + "and C1.etat in (" + etat + ")  "
                            + "and C1.datetraitement ='" + dateTraitement + "'"
                            + " and C1.remise=R.idremise "
                            + "and trim(R.etablissement) like '" + etablissementDepot.trim() + "' ";
        %>
    ...
    <div id="tableSaisieRemiseDIV" align="center">
        ...
        <form  id="remiseForm" name="remiseForm" onsubmit=" return false;">
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="numero" value="" maxlength="12" onKeypress="return numeriqueValide(event);" onblur=" chargeInfoCompteLite();" disabled/></td>
                            <% } else {%>
                        <td><input type="text" name="numero" value="${requestScope['compteRemettant']}" maxlength="12" onKeypress="return numeriqueValide(event);" onblur=" chargeInfoCompteLite();" disabled/></td>
                            <%}
                                ;%>
                        <td></td>
                        <td></td>
            ...
            <input type="hidden" value="valideRemiseLite" name="action" />
    ...
    <div id="tableSaisieChequeDIV" align="center">
        ...
        <form  id="chequeForm" name="chequeForm" onsubmit="return false;">
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td></td>
                        <td>Remise: </td>
                        <td><input type="button" name="idremise" value="${requestScope['idremise']}" onclick="location.replace('validationRemises_lite_coris.jsp?action=remise&idremise=${requestScope["idremise"]}')"/></td>
                        <td><input type="checkbox" id="escompte" name="escompte" value="OFF" />Escompte</td>
                    </tr>
            ...
            <input type="hidden" value="valideChequeLite" name="action" />
            <input type="hidden" value="${requestScope['index']}" name="index" />
        </form>
        <b>Cheque ${requestScope['index']} sur ${requestScope['nbOpers']}</b>
        <br>
        <%if (index > 1) {%>
        <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('validationRemises_lite_coris.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
        <%if (nbOpers > index) {%>
        <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('validationRemises_lite_coris.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
    </div>
    <%}
        }%>
