<%@page contentType="text/html" import="org.patware.jdbc.DataBase,java.math.BigDecimal,java.util.Date,clearing.table.Utilisateurs,clearing.web.bean.ComboCompteTPCICUBean"%>
...
<html>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <% } else {
                                comboCompteTPCICUBean.setCurrentCompteRemettant(request.getAttribute("compteRemettant").toString());
                            %>
                            <td><a:widget name="dojo.combobox" id="compteRemettant"  value="${comboCompteTPCICUBean.comboDatasWithSelected}"  publish="/cl/getCompteRemettant"/></td>
                            <%};%>
                            <td></td>
                            <td></td>
                            <td>Montant: </td>
    ...
    <div id="tableSaisieChequeDIV" align="center">
        <fieldset id="divField">
            ...
            <br>
            <%if (index > 1) {%>
            <input type="button" value="Remise" name="Remise" onclick="window.location.replace('saisiecheques_tpci_cu.jsp?action=remise&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('saisiecheques_tpci_cu.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
            <%if (nbOpers > index) {%>
            <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('saisiecheques_tpci_cu.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
        </fieldset>
    </div>
