...
<html>
    ...
    <div id="tableSaisieChequeDIV" align="center">
        ...
        <form  id="chequeForm" name="chequeForm" onsubmit="return false;">
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td></td>
                        <td>Remise: </td>
                        <td><input type="button" name="idremise" value="${requestScope['idremise']}" onclick="location.replace('validationRemises_lite_rc.jsp?action=remise&idremise=${requestScope["idremise"]}')"/></td>
                        <td></td>
                    </tr>
            ...
            <input type="hidden" value="valideChequeLiteRC" name="action" />
            <input type="hidden" value="${requestScope['index']}" name="index" />
        </form>
        <b>Cheque ${requestScope['index']} sur ${requestScope['nbOpers']}</b>
        <br>
        <%if (index > 1) {%>
        <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('validationRemises_lite_rc.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
        <%if (nbOpers > index) {%>
        <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('validationRemises_lite_rc.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
    </div>
    <%}
        }%>
