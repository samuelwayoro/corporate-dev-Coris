...
<html>
    ...
    <body>
        <%
            ...
            comboUtilisateurBean.setRequete("SELECT * FROM UTILISATEURS WHERE TRIM(LOGIN) IN (SELECT DISTINCT CODEUTILISATEUR FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAREMSAIIRIS") + " ) AND LOTSIB IS NULL AND BANQUE=BANQUEREMETTANT AND CODEREPRESENTATION IS NOT NULL)");
        ...
        <div align="center">
            <form name='choixUtilisateur'>
                ...
                <input type="button" name="startVal" value="Demarrer la Validation des Signatures des Chèques" onclick="startValidation();">
        ...
        <%} else {
            ...
            if (action != null && (action.equalsIgnoreCase("precedent") || action.equalsIgnoreCase("suivant"))) {
                ...
                String sql = "SELECT * FROM REMISES WHERE IDREMISE = (SELECT MIN(IDREMISE) FROM REMISES WHERE ETAT IN ("  + Utility.getParam("CETAREMSAIIRIS") + " ) AND (LOTAGENCE IS NULL OR LOTAGENCE IN ("  + Utility.getParam("CETAREMSIGENC")+ "))"
                        + " AND NOMUTILISATEUR LIKE '" + userSaisie.trim() + "'"
                        + " AND IDREMISE IN (SELECT REMISE FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAREMSAIIRIS") + " ) AND BANQUE=BANQUEREMETTANT AND CODEREPRESENTATION IS NOT NULL)"
                        + " AND (VALIDEUR='' OR VALIDEUR IS NULL OR VALIDEUR = '" + user.getLogin().trim() + "'))";

                Remises[] remises = (Remises[]) db.retrieveRowAsObject(sql, new Remises());
                BigDecimal lastIdSeq = null, lastId = null;
                if (remises != null && remises.length > 0) {
                    if (remises[0].getLotAgence() == null) {
                        lastDocType = "REMISE";
                    ...
                    String query = "select C1.IDCHEQUE,C1.REMISE,C1.DEVISE,C1.NUMEROCHEQUE,C1.NUMEROCOMPTE,C1.NOMBENEFICIAIRE,C1.ETAT,C1.AGENCE,C1.COMPTEREMETTANT,C1.AGENCEREMETTANT,C1.DATETRAITEMENT,C1.DATECOMPENSATION,"
                            ...
                            + "and C1.etat in (16) and R.valideur like '" + user.getLogin() + "' "
    ...
    <div align="center">
        ...
        <form name='rejetForm'>
            <div align="center">
                ...
                <input type='hidden' name='action' value='rejectDocBtnECI' />
    ...
    <div id="tableSaisieRemiseDIV" align="center">
        ...
        <form  id="remiseForm" name="remiseForm" onsubmit=" return false;">
            ...
            <input type="hidden" value="valideRemiseSignatureECI" name="action" />
    ...
    <div id="tableSaisieChequeDIV2" align="center">
        ...
        <form  id="chequeForm" name="chequeForm" onsubmit="return false;">
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td></td>
                        <td>Remise: </td>
                        <td><input type="button" name="idremise" value="${requestScope['idremise']}" onclick="location.replace('validationRemisesSignatures_eci.jsp?action=remise&idremise=${requestScope["idremise"]}')"/></td>
                        <td></td>
                    </tr>
            ...
            <input type="hidden" value="valideChequeSignatureECI" name="action" />
            <input type="hidden" value="${requestScope['index']}" name="index" />
        </form>
        <b>Cheque ${requestScope['index']} sur ${requestScope['nbOpers']}</b>
        <br>
        <%if (index > 1) {%>
        <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('validationRemisesSignatures_eci.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
        <%if (nbOpers > index) {%>
        <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('validationRemisesSignatures_eci.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
    </div>
    <%}
        }%>
