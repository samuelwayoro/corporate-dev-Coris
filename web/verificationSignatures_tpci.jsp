...
<%@page import="org.patware.xml.JDBCXmlReader, clearing.table.orion.Sibicsig,clearing.model.CMPUtility,clearing.table.Cheques,clearing.table.Banques,java.util.Date"%>
...
<body>
    <%
    clearing.table.orion.Sibicsig[] signatures = null;
    ...
    <%} else {
        ...
        if (cheques != null && cheques.length > 0) {
            ...
            signatures = CMPUtility.getOrionSignaturesModified(cheques[0].getAgence(), cheques[0].getNumerocompte());
    ...
    <div id="imageSignDIV">
        ...
        for (int i = 0; i < signatures.length; i++) {
            if (signatures[i].getNomimg().endsWith("jpg")) {
        %>
        <img name='<%="signature" + i%>' id='<%="signature" + i%>' src='<%=signatures[i].getNomimg().substring(3).replace("\\", "/")%>' width="200" height="160" alt='<%=signatures[i].getNomimg().substring(3).replace("\\", "/")%>' />
        <span> Commentaire<%=i%>  <%=signatures[i].getBanquev3()%>  </span>
        <br>
        <%} else {
            out.println(signatures[i].getBanquev3());
        }
    }%>
