...
<html>
    ...
    <body>
        <%
            ...
            comboUtilisateurBean.setRequete("SELECT * FROM UTILISATEURS WHERE ADRESSE='" + user.getAdresse().trim() + "' "
                    + " AND TRIM(LOGIN) IN (SELECT DISTINCT TRIM(VALIDEUR) FROM REMISES WHERE ETAT IN (" + Utility.getParam("CETAOPEALLICOM1") + "," + Utility.getParam("CETASUPPOUVERSIG") + "))");
        ...
        <%} else {
            ...
            if (action != null && (action.equalsIgnoreCase("precedent") || action.equalsIgnoreCase("suivant"))) {
                ...
                if (remises != null && remises.length > 0) {
                    ...
                    sql = "SELECT * FROM CHEQUES WHERE REMISE=" + curIdRemise + "  AND ETAT IN (" + Utility.getParam("CETAOPESUPVALSURCAI") + ")";;
                    Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                    if (cheques != null && cheques.length > 0) {
                ...
                String sql = "SELECT * FROM REMISES WHERE IDREMISE = (SELECT MIN(IDREMISE) FROM REMISES WHERE ETAT IN (" + Utility.getParam("CETAOPEALLICOM1") + "," + Utility.getParam("CETASUPPOUVERSIG") + "   )"
                ...
                if (remises != null && remises.length > 0) {
                    if (remises[0].getEtat().equals(new BigDecimal(Utility.getParam("CETAOPEALLICOM1")))) {
                        lastDocType = "REMISE";
                        ...
                        sql = "SELECT * FROM CHEQUES WHERE IDCHEQUE = (SELECT MIN(IDCHEQUE) FROM CHEQUES WHERE REMISE=" + remises[0].getIdremise() + " AND VILLEREMETTANT='01'  AND ETAT IN (" + Utility.getParam("CETAOPESUPVALSURCAI") + "))";
                        Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                        if (cheques != null && cheques.length > 0) {
                            lastDocType = "CHEQUE";
                    ...
                    if (lastDocType.equalsIgnoreCase("REMISE")) {
                        ...
                        if (remises != null && remises.length > 0) {
                            ...
                            request.setAttribute("reference", remises[0].getReference());
                            request.setAttribute("remarques", remises[0].getRemarques());
                            sql = "SELECT * FROM CHEQUES WHERE REMISE=" + curIdRemise + " AND ETAT IN (" + Utility.getParam("CETAOPESUPVALSURCAI") + ") ORDER BY SEQUENCE";
                            Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                            if (cheques != null && cheques.length > 0) {
                                nmourem = String.valueOf(cheques.length);
                        ...
                        sql = "SELECT * FROM CHEQUES WHERE IDCHEQUE=" + lastId + "  AND VILLEREMETTANT='01' AND ETAT =" + Utility.getParam("CETAOPESUPVALSURCAI") + " AND SEQUENCE=" + lastIdSeq;
                        Cheques[] cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                        if (cheques != null && cheques.length > 0) {
                            Banques[] banques = (Banques[]) db.retrieveRowAsObject("SELECT * FROM BANQUES WHERE CODEBANQUE=" + ((cheques[0].getBanque() == null) ? "''" : "'" + cheques[0].getBanque() + "'"), new Banques());
                            if (banques != null && banques.length > 0) {
                            ...
                            if (remises != null && remises.length > 0) {
                                session.setAttribute("currentRemiseCorr", remises[0]);
                                sql = "SELECT * FROM CHEQUES WHERE REMISE=" + cheques[0].getRemise() + " AND ETAT IN (" + Utility.getParam("CETAOPESUPVALSURCAI") + ") ORDER BY SEQUENCE";
                                Cheques[] chequesCal = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                                for (int i = 0; i < chequesCal.length; i++) {
                                    if (chequesCal[i].getIdcheque().equals(cheques[0].getIdcheque())) {
                                        index = i + 1;
                    ...
                    String interval = "User de Saisie: " + userSaisie + "| Date de Traitement du " + dateTraitement;
    ...
    <div id="tableSaisieRemiseDIV" align="center">
        ...
        <form  id="remiseForm" name="remiseForm" onsubmit=" return false;">
            ...
            <input type="hidden" value="valideRemiseLiteETG" name="action" />
    ...
    <div id="tableSaisieChequeDIV2" align="center">
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td></td>
                        <td>Remise: </td>
                        <td><input type="button" name="idremise" value="${requestScope['idremise']}" onclick="location.replace('validationRemises_signatures.jsp?action=remise&idremise=${requestScope["idremise"]}')"/></td>
                        <td></td>
                    </tr>
        ...
        <br>
        <%if (index > 1) {%>
        <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('validationRemises_signatures.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
        <%if (nbOpers > index) {%>
        <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('validationRemises_signatures.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
        <%}%>
    </div>
    <%}
        }%>
