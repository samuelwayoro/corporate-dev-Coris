...
<body onload="affiche_master();" >
    <%
        ...
        if (Utility.getParam("CODE_BANQUE_SICA3").equals("SN010")) {
            comboUtilisateurBean.setRequete("SELECT * FROM UTILISATEURS WHERE  TRIM(LOGIN) IN (SELECT DISTINCT TRIM(NOMUTILISATEUR) FROM REMISES WHERE NOMUTILISATEUR='CORPORATES' AND  ETAT= " + Utility.getParam("CETAOPEVAL") + ")");
        } else {
            comboUtilisateurBean.setRequete("SELECT * FROM UTILISATEURS WHERE ADRESSE='" + user.getAdresse().trim() + "' AND TRIM(LOGIN) IN (SELECT DISTINCT TRIM(NOMUTILISATEUR) FROM REMISES WHERE ETAT= " + Utility.getParam("CETAOPEVAL") + ")");
        }
    ...
    <%    } else {
        ...
        String corporates = "" + " AND AGENCEDEPOT='" + user.getAdresse().trim() + "'";
        if (Utility.getParam("CODE_BANQUE_SICA3") != null && Utility.getParam("CODE_BANQUE_SICA3").equals("SN010")) {
            corporates = " ";
        }
        String sql = "SELECT * FROM REMISES WHERE IDREMISE = (SELECT MIN(IDREMISE) FROM REMISES WHERE ETAT=" + Utility.getParam("CETAOPEVAL")
                + corporates
        ...
        if (remises != null && remises.length > 0) {
            ...
            String query = "select C1.IDCHEQUE,C1.REMISE,C1.DEVISE,C1.NUMEROCHEQUE,C1.NUMEROCOMPTE,C1.NOMBENEFICIAIRE,C1.ETAT,C1.AGENCE,C1.COMPTEREMETTANT,C1.AGENCEREMETTANT,C1.DATETRAITEMENT,C1.DATECOMPENSATION,"
                    ...
                    + "and R.agenceDepot like '" + user.getAdresse().trim() + "' ";
            if (Utility.getParam("CODE_BANQUE_SICA3") != null && Utility.getParam("CODE_BANQUE_SICA3").equals("SN010")) {
                query = "select C1.IDCHEQUE,C1.REMISE,C1.DEVISE,C1.NUMEROCHEQUE,C1.NUMEROCOMPTE,C1.NOMBENEFICIAIRE,C1.ETAT,C1.AGENCE,C1.COMPTEREMETTANT,C1.AGENCEREMETTANT,C1.DATETRAITEMENT,C1.DATECOMPENSATION,"
                        + "C1.banqueremettant ,B1.libellebanque as LIBELLEBANQUEREMETTANT,C2.banque ,B2.libellebanque as LIBELLEBANQUE,C1.montantcheque ,C1.MOTIFREJET "
                        + "from cheques C1,cheques C2, Banques B1,Banques B2,Remises R "
                        + "where C1.IDCHEQUE=C2.IDCHEQUE  AND C1.banqueremettant=B1.codebanque and C2.banque=B2.codebanque "
                        + "and C1.etat in (35,900)  "
                        + "and C1.datetraitement ='" + dateTraitement + "' and C1.remise=R.idremise ";
            }
