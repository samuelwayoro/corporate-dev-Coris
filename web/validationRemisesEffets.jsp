...
<%@page contentType="text/html" import="org.patware.jdbc.DataBase,java.util.Date,java.math.BigDecimal,clearing.table.Utilisateurs"%>
<%@page pageEncoding="UTF-8" import="org.patware.xml.JDBCXmlReader,org.patware.web.bean.MessageBean,org.patware.utils.Utility"%>
<%@page import="org.patware.xml.JDBCXmlReader,clearing.table.RemisesEffets,clearing.table.SequencesEffets,clearing.table.Effets,clearing.table.Banques"%>
...
<body onload="affiche_master();" >
    <%
        ...
        comboUtilisateurBean.setRequete("SELECT * FROM UTILISATEURS WHERE ADRESSE='" + user.getAdresse().trim() + "' AND TRIM(LOGIN) IN (SELECT DISTINCT TRIM(NOMUTILISATEUR) FROM REMISESEFFETS WHERE ETAT= " + Utility.getParam("CETAOPEVAL") + ")");
        db.open(JDBCXmlReader.getUrl(), JDBCXmlReader.getUser(), JDBCXmlReader.getPassword());
        
        String action = request.getParameter("choixUtilisateurEffet");
        if (action != null && action.equalsIgnoreCase("true")) {
			%>

    <div align="center">
        <form name='choixUtilisateurEffet'>
            Choisissez un utilisateur : <a:widget name="dojo.combobox" id="user" value="${comboUtilisateurBean.comboDatas}"/>
            <input type="button" name="startVal" value="Demarrer la Validation des Remises Effets" onclick="startValidationEffet();">
            <input type="hidden" name="utilisateur" value=""/>
            <input type="hidden" name="action" value="choixUtilisateurEffet"/>
        </form>
        <br>
        <br>
            <br>
...
<tr>
    ...
    <%    } else {
        ...
        String sql = "SELECT * FROM REMISESEFFETS WHERE IDREMISE = (SELECT MIN(IDREMISE) FROM REMISESEFFETS WHERE ETAT=" + Utility.getParam("CETAOPEVAL") +
        ...
        if (remisesEffets != null && remisesEffets.length > 0) {
            ...
            BigDecimal curIdRemise = remisesEffets[0].getIdremise();
            String fPicture = "";
            String rPicture = "";
            String fPictureCheque = "";
            String rPictureCheque = "";
            request.setAttribute("idObjet", curIdRemise);
            if (remisesEffets[0].getPathimage() != null) {
                fPicture = remisesEffets[0].getPathimage().substring(3) + "\\" + remisesEffets[0].getFichierimage() + "f.jpg";
                rPicture = remisesEffets[0].getPathimage().substring(3) + "\\" + remisesEffets[0].getFichierimage() + "r.jpg";
            }
            request.setAttribute("fPicture", fPicture.replace("\\", "/"));
            request.setAttribute("rPicture", rPicture.replace("\\", "/"));
            request.setAttribute("montantRemise", remisesEffets[0].getMontant());
            request.setAttribute("compteRemettant", remisesEffets[0].getCompteRemettant());
            sql = "UPDATE REMISESEFFETS SET VALIDEUR='" + user.getLogin().trim() + "' WHERE IDREMISE=" + curIdRemise;
            ...
            if (effets != null && effets.length > 0) {
                fPictureCheque = effets[0].getPathimage().substring(3) + "\\" + effets[0].getFichierimage() + "f.jpg";
                rPictureCheque = effets[0].getPathimage().substring(3) + "\\" + effets[0].getFichierimage() + "r.jpg";
            }
            ...
            sql = "SELECT IDREMISE,COMPTEREMETTANT as \"COMPTE REMETTANT\",NOMCLIENT,MONTANT,NBOPERATION,DATESAISIE,AGENCEREMETTANT," +
                    "NOMUTILISATEUR AS \"UTILISATEUR SAISIE\",REMARQUES AS \"REMARQUES.\",PATHIMAGE,FICHIERIMAGE FROM REMISESEFFETS " +
                    "WHERE IDREMISE =" + curIdRemise;
            request.setAttribute("filtre", URLEncoder.encode(sql, "UTF-8"));
            sql = "SELECT IDEFFET as \"IDEFFET\",NUMEROEFFET, MONTANT_EFFET, BANQUE,AGENCE,NUMEROCOMPTE_TIRE, RIBCOMPTE,DATE_CREATION as \"DATE CREATION\",DATE_ECHEANCE as \"DATE ECHEANCE\",PATHIMAGE,FICHIERIMAGE,DATETRAITEMENT FROM EFFETS " +
            ...
            finish = true;
            out.println("Vous n'avez plus de remises effets à valider, merci"+ "<BR>");
            out.println("Voulez-vous imprimer votre rapport quotidien?"+ "<BR>");
        ...
        String query = "select C1.IDCHEQUE,C1.REMISE,C1.REFREMISE,C1.DEVISE,C1.NUMEROCHEQUE,C1.NUMEROCOMPTE,C1.NOMBENEFICIAIRE,C1.ETAT,C1.AGENCE,C1.COMPTEREMETTANT,C1.AGENCEREMETTANT,C1.DATETRAITEMENT,C1.DATECOMPENSATION," +
                ...
                "from cheques C1,cheques C2, Banques B1,Banques B2,Remiseseffets R " +
    ...
    <DIV  >
        ...
        <a:widget name="yahoo.dataTable" id="remiseseffets" args="{paginated:false}"  service="data.jsp?requete=${requestScope['filtre']}&objet=REMISESEFFETS&nomidobjet=IDREMISE"
                  />
    ...
    <form name="detailForm" id="detailForm">
        <div id="monitorPanelDIV">
            <input type='button' name='valider' value='Valider' onclick=" validerRemiseEffet();"/>
            <input type='button' name='rejeter' value='Rejeter' onclick=" rejeterRemiseEffet();"/>
        </div>
        ...
        <input type="hidden" name="idObjet" value="${requestScope['idObjet']}"/>
         ...
         <input type="hidden" name="action" value="updateDataTableDevEffet"/>
        ...
        <input type="hidden" name="table" value="REMISESEFFETS"/>
    ...
    <DIV  >
        ...
        <a:widget name="yahoo.dataTable" id="tableCheques" args="{scrollable:true}" service="data.jsp?requete=${requestScope['filtre1']}&objet=EFFETS&nomidobjet=IDEFFET" publish="/jmaki/tableCheques/onClick"/>
