...
<html>
            ...
            if (params_auto == null || (params_auto != null && params_auto.equalsIgnoreCase("1"))) {
                ...
                if (action != null && (("precedent|remise").contains(action) || action.equalsIgnoreCase("suivant"))) {
                    ...
                    if (sequences != null && sequences.length > 0) {
                        ...
                        String query = "select C1.IDCHEQUE,C1.NOMEMETTEUR,C1.REMISE,C1.REFREMISE,C1.DEVISE,C1.NUMEROCHEQUE,C1.NUMEROCOMPTE,C1.NOMBENEFICIAIRE,C1.ETAT,C1.AGENCE,C1.COMPTEREMETTANT,C1.AGENCEREMETTANT,C1.DATETRAITEMENT,C1.DATECOMPENSATION,"
                        ...
                        String query2 = "SELECT B.LIBELLEBANQUE AS LIBELLEBANQUE, C.NOMEMETTEUR AS TIREUR,C.NUMEROCHEQUE AS NUMEROCHEQUE, C.DATEEMISSION AS DATEEMISSION,"
        ...
        <form name="printReport"  action="ControlServlet" method="POST" onsubmit="return resolveQuotidien();">
            <table>
                ...
                <tr>
                    ...
                    <td><input type="radio" id="orderbyremisesigne" name="nomrapport" value="Cheques_clients_remises_signe" checked="checked"/>Regroupé Par Remise pour signature</td>
                </tr>
                <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="reference" value="" maxlength="25" /></td>
                                <% } else {%>
                            <td><input type="text" name="reference" value="${requestScope['reference']}" maxlength="25" /></td>
                                <%}%>
                            <td></td>
    ...
    <div id="tableSaisieChequeDIV" align="center">
        <fieldset id="divField">
            ...
            <br>
            <%if (index > 1) {%>
            <input type="button" value="Remise" name="Remise" onclick="window.location.replace('saisiecheques_lite.jsp?action=remise&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('saisiecheques_lite.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
            <%if (nbOpers > index) {%>
            <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('saisiecheques_lite.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
        </fieldset>
    </div>
