...
<html>
                ...
                if (action != null && (("precedent|remise").contains(action) || action.equalsIgnoreCase("suivant"))) {
                    ...
                    if (sequences != null && sequences.length > 0) {
                        ...
                        if (Utility.getParam("VALIDATION_CHEQUE_AUTO") != null && Utility.getParam("VALIDATION_CHEQUE_AUTO").equalsIgnoreCase("AUTO")) {
                            etat = Utility.getParam("CETAOPEALLICOM1") + "," + Utility.getParam("CETAOPEALLICOM1");
                        } else {
    ...
    <div id="tableSaisieChequeDIV" align="center">
        <fieldset id="divField">
            ...
            <br>
            <%if (index > 1) {%>
            <input type="button" value="Remise" name="Remise" onclick="window.location.replace('saisiecheques_esn.jsp?action=remise&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <input type="button" value="Precedent" name="Précédent" onclick="window.location.replace('saisiecheques_esn.jsp?action=precedent&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
            <%if (nbOpers > index) {%>
            <input type="button" value="Suivant" name="Suivant" onclick="window.location.replace('saisiecheques_esn.jsp?action=suivant&idcheque=${requestScope['idObjet']}&idremise=${requestScope['idremise']}')"/>
            <%}%>
        </fieldset>
    </div>
