...
<jsp:include page="checkaccess.jsp"/>
<jsp:useBean id="comboAgenceBean" class="clearing.web.bean.ComboAgenceBean" />
<body>
    <%
        DataBase db = new DataBase(JDBCXmlReader.getDriver());
        Utilisateurs user = (Utilisateurs) session.getAttribute("utilisateur");

        String agenceChoisie = (String) session.getAttribute("agenceChoisie");
        if (agenceChoisie == null) {
            agenceChoisie = "%";
        }
        comboAgenceBean.setRequete("select * from agences where codeagence in (select distinct agence from cheques where etat in (" + Utility.getParam("CETAOPERETRECENVSIB") + ") and LOTSIB IN (1,2)) and codebanque like '" + CMPUtility.getCodeBanqueSica3() + "'");
        db.open(JDBCXmlReader.getUrl(), JDBCXmlReader.getUser(), JDBCXmlReader.getPassword());
        String action = request.getParameter("choixAgences");
        if (action != null && action.equalsIgnoreCase("true")) {%>

    <div align="center">
        <form name='choixCodeAgences'>
            Choisissez une Agence : <a:widget name="dojo.combobox" id="agences" value="${comboAgenceBean.comboDatas}"/>
            <input type="button" name="startVal" value="Demarrer la Vérification des Signatures" onclick="startValidationParAgence();">
            <input type="hidden" name="codeAgence" value=""/>
            <input type="hidden" name="action" value="choixCodeAgences"/>
        </form>
    </div>
    <%} else {
        ...
        if (cheques != null && cheques.length > 0) {
            ...
            request.setAttribute("numeroCheque", Utility.formatNumber(cheques[0].getNumerocheque()));

            sql = "UPDATE CHEQUES SET CODEUTILISATEUR='" + user.getLogin().trim() + "' WHERE IDCHEQUE=" + curIdCheque;
            db.executeUpdate(sql);

            sql = "SELECT IDCHEQUE as \"IDCHEQUE\",NUMEROCHEQUE, MONTANTCHEQUE, BANQUE, A.AGENCE,NUMEROCOMPTE, RIBCOMPTE,IBAN AS \"REMARQUES.\", DATETRAITEMENT,NOMBENEFICIAIRE,NOM as \"NOMTIRE\", BANQUEREMETTANT "
                    + " FROM CHEQUES  A FULL OUTER JOIN COMPTES B ON A.NUMEROCOMPTE=B.NUMERO AND A.AGENCE=B.AGENCE "
    ...
    <div id="imageDIV2" >
        <form name='imgChqForm'>
            ...
            <br>
            Num:${requestScope['numeroCheque']}&nbsp;&nbsp;&nbsp; Mnt:${requestScope['montantCheque']}
            <br>
    ...
    <div id="imageSignDIV">
        ...
        <table>
            ...
            <%
                        ...
                        out.println("<tr><td align='center'>" + signatures[i].substring(signatures[i].lastIndexOf("_") - 1, signatures[i].lastIndexOf("_")) + "</td></tr>");
                        out.println("</table>");
                    } else {
