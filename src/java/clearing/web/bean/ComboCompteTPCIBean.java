...
import clearing.table.orion.Sibiccpt;
 ...
 * @author Patrick
 */
public class ComboCompteTPCIBean extends ComboBean {
    ...
    public String getInfoCompte(String numeroCompte) {
        try {
            ...
            Sibiccpt[] sibiccpt = (Sibiccpt[]) dbExt.retrieveRowAsObject(requete + " where compte like '" + Utility.bourrageGauche(numeroCompte, 12, "0") + "'", new Sibiccpt());


            if (sibiccpt != null && sibiccpt.length > 0) {
                sibiccpt[0].setNomabreg(sibiccpt[0].getTitulair().replaceAll("\\p{Punct}", " "));
                ...
                compteMessageBean.setNomClient(Utility.bourrageDroite((sibiccpt[0].getNomabreg()!=null?sibiccpt[0].getNomabreg().trim():"") + " " + (sibiccpt[0].getPrenom()!=null?sibiccpt[0].getPrenom().trim():""), 35, " "));
                compteMessageBean.setAgence(sibiccpt[0].getGuichet());
     ...
     public String getInfoCompte( String agence, String numeroCompte) {
        try {
            ...
            if (sibiccpt != null && sibiccpt.length > 0) {
                ...
                CompteMessageBean compteMessageBean = new CompteMessageBean();
                compteMessageBean.setNumeroCompte(sibiccpt[0].getCompte());
                compteMessageBean.setNomClient(Utility.bourrageDroite(sibiccpt[0].getTitulair().trim() + " " + sibiccpt[0].getPrenom().trim(), 35, " "));
