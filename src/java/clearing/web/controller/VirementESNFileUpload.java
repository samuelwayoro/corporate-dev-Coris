...
import clearing.action.readers.flexcube.esn.VirementESNXLSReader;
 ...
 * @author DavyStephane
 */
public class VirementESNFileUpload extends HttpServlet {
            ...
            try {
                ...
                for (FileItem item : items) {
                    if (!item.isFormField()) {
                        ...
                        VirementESNXLSReader readerVirement = new VirementESNXLSReader();
                        Repertoires repertoire = new Repertoires();
                        repertoire.setChemin(destPath);
                        repertoire.setExtension("xls");
                        repertoire.setTache("clearing.action.readers.flexcube.esn.VirementESNXLSReader");
...
//                        fichier.setParam2(fieldsMap.get("param2"));
                        ...
                        message.append("Il y a  ").append(nbVirements).append(" virements dans le fichier intégré pour un montant total de : ").append(number_fr).append(" XOF");
...
//                        req.getRequestDispatcher("/resultUploadVirements.jsp").forward(req, resp);
            ...
            if (action.equals("OUI")) {
                try {
                    //Fichier confirmé. inserer les prelevements en BD
                    //Connection a la bd
                    DataBase db = new DataBase(JDBCXmlReader.getDriver());
                    db.open(JDBCXmlReader.getUrl(), JDBCXmlReader.getUser(), JDBCXmlReader.getPassword());
                ...
                resp.sendRedirect("uploadVirements_ESN.jsp");
                session.removeAttribute("resultat");
...
//                    db.insertObjectAsRowByQuery(f, "FICHIERS");
                ...
                req.getRequestDispatcher("/uploadVirements_ESN.jsp").forward(req, resp);
            }
            session.removeAttribute("virementsAttribute");
            session.removeAttribute("fileTreated");
            session.removeAttribute("message");
