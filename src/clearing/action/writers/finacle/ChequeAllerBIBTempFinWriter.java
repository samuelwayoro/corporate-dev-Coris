 ...
 * @author Patrick
 */
public class ChequeAllerBIBTempFinWriter extends FlatFileWriter {

    public ChequeAllerBIBTempFinWriter() {
        ...
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i += j) {
                ...
                if((remises != null && 0 < remises.length) &&
                   (allChequesVal.length == remises[0].getNbOperation().intValue())){
                    ...
                    if (aCheque.getEscompte().intValue()==1){
                        line =  Utility.bourrageGauche(aCheque.getCompteremettant(), 12, "0")+
                                createBlancs(4, " ")+"XOF"+
                                Utility.bourrageGauche(""+sumRemise, 17, "0")+" "+
                                Utility.bourrageDroite(aCheque.getNombeneficiaire(),30, " ")+
                                Utility.bourrageGauche(remises[0].getReference(), 7, "0")+" "+
                                Utility.bourrageGauche(""+sumRemise, 17, "0")+
                                createBlancs(12, " ")+
                                //aCheque.getAgence()+" "+
                                CMPUtility.getAgencePrincipale(aCheque.getBanque())+" "+
                                Utility.getParamNameOfType(aCheque.getBanque().substring(0, 2), "CODE_CORRESPONDANCE")+aCheque.getBanque().substring(2, 5)+" "+
                                "Y"+createBlancs(16, " ");
                   ...
                   db.executeUpdate("UPDATE CHEQUES SET ETAT ="+ Utility.getParam("CETAOPEALLICOM1")+" WHERE REMISE="+ aCheque.getRemise()+ " AND BANQUE NOT IN ('" + CMPUtility.getCodeBanque() + "','" + CMPUtility.getCodeBanqueSica3() + "')");
