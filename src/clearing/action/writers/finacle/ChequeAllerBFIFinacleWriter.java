 ...
 * @author Patrick
 */
public class ChequeAllerBFIFinacleWriter extends FlatFileWriter {

    public ChequeAllerBFIFinacleWriter() {
...
////        
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i += j) {
                ...
                sql = "SELECT * FROM CHEQUES WHERE REMISE=" + cheques[i].getRemise() + " AND ETAT IN (" + Utility.getParam("CETAOPEVAL2") + "  , " + Utility.getParam("CETAOPESUPVALSURCAI") + ")";
...
//                                    + ((i == cheques.length - 1) ? "Y" : "N") + createBlancs(433, " ");
            ...
            setDescription(getDescription() + " exécuté avec succès: Nombre de Chèques= " + cheques.length + " - Montant Total= " + montantTotal + " Nom de Fichier :" + fileName);
            logEvent("INFO", "Nombre de Chèques= " + cheques.length + " - Montant Total= " + montantTotal+ " Nom de Fichier :" + fileName);
            closeFile();
        } else {
