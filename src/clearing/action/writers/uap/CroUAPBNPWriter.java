 ...
 * @author Patrick
 */
public class CroUAPBNPWriter extends FlatFileWriter {
    ...
    public CroUAPBNPWriter() {
        setDescription("Envoi de Lot d'operation retour vers le SIB");
    }
    ...
    public void execute() throws Exception {
        ...
        String whereClause = " ETAT IN (" + Utility.getParam("CETAOPERETREC") + "," + Utility.getParam("CETAOPERET") + ") AND LOTSIB IS NULL AND BANQUEREMETTANT LIKE '" + Utility.getParam("CODE_BANQUE_SICA3").substring(0, 2) + "%' ";
        ...
        whereClause = " ETAT IN (" + Utility.getParam("CETAOPEREJRET") + ") AND LOTSIB IS NULL AND BANQUE LIKE '" + Utility.getParam("CODE_BANQUE_SICA3").substring(0, 2) + "%' ";
        ...
        whereClause = " ETAT IN (" + Utility.getParam("CETAOPERETREC") + "," + Utility.getParam("CETAOPERET") + ") AND LOTSIB IS NULL AND BANQUEREMETTANT NOT LIKE '" + Utility.getParam("CODE_BANQUE_SICA3").substring(0, 2) + "%' ";
        ...
        whereClause = " ETAT IN (" + Utility.getParam("CETAOPEREJRET") + ") AND LOTSIB IS NULL AND BANQUE NOT LIKE '" + Utility.getParam("CODE_BANQUE_SICA3").substring(0, 2) + "%' ";
        //Recuperation des rejets cheques 130
        sql = "SELECT * FROM CHEQUES WHERE " + whereClause + " AND TYPE_CHEQUE ='030'";
        cheques = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
        prepareRejetCheques(cheques);
        ...
        sql = "SELECT * FROM CHEQUES WHERE " + whereClause + " AND TYPE_CHEQUE ='035'";
        cheques35 = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
        if (cheques35 != null && cheques35.length > 0) {
            prepareRejetCheques(cheques35);
        }
        ...
        sql = "SELECT * FROM EFFETS WHERE   " + whereClause + " AND TYPE_EFFET ='040' ";
        effets40 = (Effets[]) db.retrieveRowAsObject(sql, new Effets());
        if (effets40 != null && effets40.length > 0) {
            prepareRejetEffets(effets40);
        }
        ...
        sql = "SELECT * FROM EFFETS WHERE  " + whereClause + " AND TYPE_EFFET ='045'";
        effets45 = (Effets[]) db.retrieveRowAsObject(sql, new Effets());
        if (effets45 != null && effets45.length > 0) {
            prepareRejetEffets(effets45);
        }
        ...
        effets46 = (Effets[]) db.retrieveRowAsObject(sql, new Effets());
    //    prepareRejetEffets(effets46);
        if (effets46 != null && effets46.length > 0) {
            prepareRejetEffets(effets46);
        }
        //Recuperation des effets d'une Banque
        sql = "SELECT * FROM EFFETS  WHERE" + whereClause + " AND TYPE_EFFET ='042' ";
        ...
        if (refLot > 0) {
            ...
            for (int i = 0; i < vEnteteLots.size(); i++) {
                logEvent("INFO", "Nombre d'operations " + vEnteteLots.get(i).getTypeOperation() + " -> " + vEnteteLots.get(i).getNbOperations());
            }
    ...
    private void feedOperationWithVirement15(Virements virement, int j, Operation[] operations) {
        ...
        if (Long.parseLong(virement.getNumerocompte_Tire()) == 0) {
            operations[j].setRibDebiteur(createBlancs(24, " "));
        } else {
            operations[j].setRibDebiteur(virement.getBanqueremettant() + virement.getAgenceremettant() + virement.getNumerocompte_Tire() + Utility.computeCleRIB(virement.getBanqueremettant(), virement.getAgenceremettant(), virement.getNumerocompte_Tire()));
        }
    ...
    private void preparePrelevements(Prelevements[] prelevements) throws Exception {
        if (prelevements != null && prelevements.length > 0) {
            ...
            enteteLot.setRefLot(Utility.computeCompteur("REFLOTPRE", CMPUtility.getDate()));
    ...
    private void prepareRejetPrelevements(Prelevements[] prelevements) throws Exception {
        if (prelevements != null && prelevements.length > 0) {
            ...
            enteteLot.setIdEntete("ECRO");
            ++refLot;
            enteteLot.setRefLot(Utility.computeCompteur("REFLOTREJPRE", CMPUtility.getDate()));
            enteteLot.setRefBancaire(enteteLot.getRefLot());
    ...
    private void prepareRejetCheques(Cheques[] cheques) throws Exception {
        ...
        if (cheques != null && cheques.length > 0) {
            ...
            enteteLot.setIdEntete("ECRO");
            ++refLot;
            enteteLot.setRefLot(Utility.computeCompteur("REFLOTREJCHE", CMPUtility.getDate()));
            enteteLot.setRefBancaire(enteteLot.getRefLot());
    ...
    private void prepareRejetEffets(Effets[] effets) throws Exception {
        if (effets != null && effets.length > 0) {
            ...
            enteteLot.setIdEntete("ECRO");
            ++refLot;
            enteteLot.setRefLot(Utility.computeCompteur("REFLOTREJEFF", CMPUtility.getDate()));
            enteteLot.setRefBancaire(enteteLot.getRefLot());
    ...
    private void prepareCheques(Cheques[] cheques) throws Exception {
        ...
        if (cheques != null && cheques.length > 0) {
            ...
            ++refLot;
            enteteLot.setRefLot(Utility.computeCompteur("REFLOTCHE", CMPUtility.getDate()));
    ...
    private void prepareEffets(Effets[] effets) throws Exception {
        if (effets != null && effets.length > 0) {
            ...
            enteteLot.setIdEntete("ECRO");
            ++refLot;
            enteteLot.setRefLot(Utility.computeCompteur("REFLOTEFF", CMPUtility.getDate()));
            enteteLot.setRefBancaire(enteteLot.getRefLot());
    ...
    private void prepareVirements(Virements[] virements) throws Exception {
        if (virements != null && virements.length > 0) {
            ...
            enteteLot.setIdEntete("ECRO");
            ++refLot;
            enteteLot.setRefLot(Utility.bourrageGZero(Utility.computeCompteur("REFLOTVIR" + virements[0].getType_Virement(), CMPUtility.getDate()), 3));
    ...
    private String printCheques35(EnteteLot enteteLot, int j) {
        ...
        line = enteteLot.operations[j].getRio().getRio()
                + enteteLot.operations[j].getTypeOperation()
                + enteteLot.operations[j].getRefOperation()
                + enteteLot.operations[j].getIdAgeRem()
...
//            db.executeUpdate(l_sql);
            l_sql = "UPDATE CHEQUES SET LOTSIB=" + "1" + " WHERE ETAT = " + Utility.getParam("CETAOPERETREC");
            db.executeUpdate(l_sql);
            //MAJ DES CHEQUES SANS IMAGES AVEC MOTIF REJET 215
            l_sql = "UPDATE CHEQUES SET LOTSIB=" + "1" + " WHERE ETAT =" + Utility.getParam("CETAOPERET");
            db.executeUpdate(l_sql);

        }

        if (remiseHasRejetsCheques) {
            l_sql = "UPDATE CHEQUES SET LOTSIB=" + "1" + " WHERE ETAT = " + Utility.getParam("CETAOPEREJRET");
            db.executeUpdate(l_sql);

        }
        if (remiseHasRejetsPrelevements) {
            l_sql = "UPDATE PRELEVEMENTS SET LOTSIB=1, ETAT=" + Utility.getParam("CETAOPEREJRETENVSIB") + " WHERE ETAT = " + Utility.getParam("CETAOPEREJRET");
            db.executeUpdate(l_sql);

        }

        if (remiseHasRejetsEffets) {
            l_sql = "UPDATE EFFETS SET LOTSIB=1, ETAT=" + Utility.getParam("CETAOPEREJRETENVSIB") + " WHERE ETAT = " + Utility.getParam("CETAOPEREJRET");
            db.executeUpdate(l_sql);

        }

        if (remiseHasVirements) {
            l_sql = "UPDATE VIREMENTS SET LOTSIB=1, ETAT=" + Utility.getParam("CETAOPERETENVSIB") + " WHERE ETAT = " + Utility.getParam("CETAOPERET");
            db.executeUpdate(l_sql);
            l_sql = "UPDATE VIREMENTS SET LOTSIB=" + "1" + " WHERE ETAT = " + Utility.getParam("CETAOPERETREC");
            db.executeUpdate(l_sql);

        }

        if (remiseHasEffets) {
            l_sql = "UPDATE EFFETS SET LOTSIB=" + "1" + " WHERE ETAT = " + Utility.getParam("CETAOPERET");
            db.executeUpdate(l_sql);
            l_sql = "UPDATE EFFETS SET LOTSIB=" + "1" + " WHERE ETAT = " + Utility.getParam("CETAOPERETREC");
            db.executeUpdate(l_sql);
        }

        if (remiseHasPrelevements) {
            l_sql = "UPDATE PRELEVEMENTS SET LOTSIB=1, ETAT=" + Utility.getParam("CETAOPERETENVSIB") + " WHERE ETAT = " + Utility.getParam("CETAOPERET");
            db.executeUpdate(l_sql);
            l_sql = "UPDATE PRELEVEMENTS SET LOTSIB=" + "1" + " WHERE ETAT = " + Utility.getParam("CETAOPERETREC");
            db.executeUpdate(l_sql);
        }
