 ...
 * @author Patrick
 */
public class ENVPrelevementAllerRepWriter extends FlatFileWriter{

    public ENVPrelevementAllerRepWriter() {
    }
    ...
    @Override
       public void execute() throws Exception {
        ...
        String fileName = Utility.getParam("SIB_IN_FOLDER") + File.separator + "01-GN-"+ Utility.getParam("CODE_BANQUE")+"-"+ dateFichier
                          + "-" + heureFichier +"-23-21-324.ENV";
        setOut(createFlatFile(fileName));

        String sql = "SELECT * FROM PRELEVEMENTS WHERE ETAT IN (" +  Utility.getParam("CETAOPEALLICOM1REP")  + ") ";
        ...
        long sumVirements = 0;
         ...
         String line = "101GN";
            ...
            for (int i = 0; i < prelevements.length; i ++) {
                ...
                line += Utility.convertDateToString(new Date(System.currentTimeMillis()), "ddMMyyyy");
                line += heureFichier;
                line += "23"+ Utility.getParam("CODE_BANQUE");
                line += dateCompensation+dateCompensation;
                line += numRemise;
                line += "21"+"324";
                ...
                wwriteln(line);
                if(prelevement.getEtat().toPlainString().equals(Utility.getParam("CETAOPEALLICOM1REP"))){
                    prelevement.setEtat(new BigDecimal(Utility.getParam("CETAOPEALLICOM1REPACC")));
