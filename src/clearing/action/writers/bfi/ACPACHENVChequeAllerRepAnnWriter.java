...
/**
 * -	Remises de présentation d'opérations : Code enregistrement 21 ; CHEQUES
 *
 * @author Patrick
 */
public class ACPACHENVChequeAllerRepAnnWriter extends FlatFileWriter {

    public ACPACHENVChequeAllerRepAnnWriter() {
        setDescription("Envoi des Cheques   Aller  Rep. Ann vers SYSTAC");
    }
...
//                + "-" + heureFichier + "-33-24-950.ENV";
        String sql = " SELECT * FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAOPEALLREPANN") + ") "
                + " AND BANQUE<>BANQUEREMETTANT     AND  (BANQUE LIKE '1%' OR BANQUE LIKE '91%' )  "; // CETAOPEALLREPANN
        ...
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i++) {
                ...
                wwriteln(line);
                if (cheque.getEtat().toPlainString().equals(Utility.getParam("CETAOPEALLREPANN"))) {
                    cheque.setEtat(new BigDecimal(Utility.getParam("CETAOPEALLREPANNACC"))); // CETAOPEALLREPANN CETAOPEALLICOM1REPACC
            ...
            setDescription(getDescription() + " exécuté avec succès:\n Nombre de Cheques= " + cheques.length + " - Montant Total= " + Utility.formatNumber("" + sumCheques) + " - Nom de Fichier = " + fileName);
            logEvent("INFO", "Nombre de Cheques= " + cheques.length + " - Montant Total= " + Utility.formatNumber("" + sumCheques));
            closeFile();
        } else {
