 ...
 * @author Patrick
 */
public class ENVVirementAllerRepWriter extends FlatFileWriter{

    public ENVVirementAllerRepWriter() {
    }
    ...
    @Override
       public void execute() throws Exception {
        ...
        String fileName = Utility.getParam("SIB_IN_FOLDER") + File.separator + "01-GN-"+ Utility.getParam("CODE_BANQUE")+"-"+ dateFichier
                          + "-" + heureFichier +"-13-21-324.ENV";
        setOut(createFlatFile(fileName));

        String sql = "SELECT * FROM VIREMENTS WHERE ETAT IN (" +  Utility.getParam("CETAOPEALLICOM1REP")  + ") ";
        ...
        long sumVirements = 0;
         ...
         String line = "101GN";
            ...
            for (int i = 0; i < virements.length; i ++) {
                ...
                line += Utility.convertDateToString(new Date(System.currentTimeMillis()), "ddMMyyyy");
                line += heureFichier;
                line += "13"+ Utility.getParam("CODE_BANQUE");
                line += dateCompensation+dateCompensation;
                line += numRemise;
                line += "21"+"324"+"00"+"0";
                ...
                wwriteln(line);
                if(virement.getEtat().toPlainString().equals(Utility.getParam("CETAOPEALLICOM1REP"))){
                    virement.setEtat(new BigDecimal(Utility.getParam("CETAOPEALLICOM1REPACC")));
