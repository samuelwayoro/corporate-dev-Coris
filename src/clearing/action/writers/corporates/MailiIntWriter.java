 ...
 * @author Patrick
 */
public class MailiIntWriter extends BinFileWriter {
    ...
    public void execute() throws Exception {
        ...
        if (parRemise != null && parRemise.equals("1")) {
            sql = " SELECT * FROM REMISES WHERE IDREMISE IN (SELECT DISTINCT REMISE FROM CHEQUES WHERE ETAT = " + Utility.getParam("CETAOPEVALDELTA") + ")";
            Remises remises[] = (Remises[]) db.retrieveRowAsObject(sql, new Remises());
            if (remises != null) {
                for (int i = 0; i < remises.length; i++) {
                    Remises remise = remises[i];
                    sql = "SELECT * FROM CHEQUES WHERE ETAT = " + Utility.getParam("CETAOPEVALDELTA") + " AND TYPE_CHEQUE ='035' AND REMISE LIKE '" + remise.getIdremise() + "' AND ETATIMAGE = " + Utility.getParam("CETAIMASTO");
...
//       
            ...
            if (banques != null) {
                for (int i = 0; i < banques.length; i++) {
                    ...
                    sql = "SELECT * FROM CHEQUES WHERE ETAT = " + Utility.getParam("CETAOPEVALDELTA") + " AND TYPE_CHEQUE ='035' AND BANQUE LIKE '" + banque.getCodebanque() + "' AND ETATIMAGE = " + Utility.getParam("CETAIMASTO");
                    cheques35 = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                    prepareCheques(cheques35, db);
                    nbCheques += cheques35.length;
...
//            sequence = Utility.bourrageGZero(Utility.computeCompteur("MAILI_INT", Utility.getParam("DATECOMPENS_NAT")), 3);
            ...
            for (int j = 0; j < cheques.length; j++) {
                ...
                // imageIndex.setReferenceRemise("" + cheque.getRemise());
                String paramRefExterneCheque = Utility.getParam("CORP_REFEXTCHE");
                String iban = cheque.getIban();
                if (iban == null || !iban.isEmpty()) {
                    iban = " ";
                }
                if (paramRefExterneCheque != null && paramRefExterneCheque.equals("1")) {
                    // Police  ou ref Specifique du cheque
                    imageIndex.setReferenceRemise("" + iban.trim());
                } else {
                    imageIndex.setReferenceRemise("" + iban.trim());
                }
