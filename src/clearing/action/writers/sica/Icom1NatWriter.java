 ...
 * @author Patrick
 */
public class Icom1NatWriter extends FlatFileWriter {
    ...
    public Icom1NatWriter() {
        setDescription("Envoi ICOM1 National vers la BCEAO ");
    }
    ...
    public void execute() throws Exception {
        ...
        if (Utility.getParam("ENVOI_ICOM_NAT") == null || Utility.getParam("ENVOI_ICOM_NAT").equals("0")) {
            if (Utility.getParam("ATTENTE_ICOMA_NAT") == null || Utility.getParam("ATTENTE_ICOMA_NAT").equals("0")) {
                ...
                Banques banques[] = CMPUtility.getBanquesNationales(db, " ETAT = " + Utility.getParam("CETAOPEALLICOM1"));
            ...
            logEvent("WARNING", "Un ICOM1 est en cours d'envoi|ENVOI_ICOM_NAT=1");
            setDescription(getDescription() + " - WARNING: Un ICOM1 est en cours d'envoi|ENVOI_ICOM_NAT=1");
        }
    ...
    private void feedOperationWithVirement15(Virements virement, int j, Operation[] operations) {
        ...
        operations[j].setRibCrediteur(virement.getBanque() + virement.getAgence() + Utility.bourrageGZero(virement.getNumerocompte_Beneficiaire(), 12)
                + Utility.computeCleRIB(virement.getBanque(), virement.getAgence(), Utility.bourrageGZero(virement.getNumerocompte_Beneficiaire(), 12)));
    ...
    private void updateEtatOperations(DataBase db) throws SQLException {
        ...
        db.executeUpdate(l_sql);
        if (remiseHasCheques) {
            l_sql = "UPDATE CHEQUES SET DATECOMPENSATION='" + Utility.convertDateToString(Utility.convertStringToDate(Utility.getParam("DATECOMPENS_NAT"), "yyyyMMdd"), ResLoader.getMessages("patternDate")) + "',ETAT=" + Utility.getParam("CETAOPEALLICOM1ENV") + " WHERE ETAT =" + Utility.getParam("CETAOPEALLPREICOM1");
            db.executeUpdate(l_sql);
        }

        if (remiseHasVirements) {
            l_sql = "UPDATE VIREMENTS SET DATECOMPENSATION='" + Utility.convertDateToString(Utility.convertStringToDate(Utility.getParam("DATECOMPENS_NAT"), "yyyyMMdd"), ResLoader.getMessages("patternDate")) + "', ETAT=" + Utility.getParam("CETAOPEALLICOM1ENV") + " WHERE ETAT =" + Utility.getParam("CETAOPEALLPREICOM1");
            db.executeUpdate(l_sql);
        }

        if (remiseHasEffets) {
            l_sql = "UPDATE EFFETS SET DATECOMPENSATION='" + Utility.convertDateToString(Utility.convertStringToDate(Utility.getParam("DATECOMPENS_NAT"), "yyyyMMdd"), ResLoader.getMessages("patternDate")) + "',ETAT=" + Utility.getParam("CETAOPEALLICOM1ENV") + " WHERE ETAT =" + Utility.getParam("CETAOPEALLPREICOM1");
            db.executeUpdate(l_sql);
        }
