 ...
 * @author Patrick
 */
public class Icom2SrgWriter extends FlatFileWriter {
    ...
    public Icom2SrgWriter() {
        setDescription("Envoi ICOM2 Sous Regional vers la BCEAO ");
    }
    ...
    public void execute() throws Exception {
        ...
        if (Utility.getParam("ENVOI_ICOM_SRG") == null || Utility.getParam("ENVOI_ICOM_SRG").equals("0")) {
            if (Utility.getParam("ATTENTE_ICOMA_SRG") == null || Utility.getParam("ATTENTE_ICOMA_SRG").equals("0")) {
                ...
                //Recuperation des Banques SOUS REGIONALES
                Banques banques[] = CMPUtility.getBanquesRemettantesSousRegionales(db, " ETAT = " + Utility.getParam("CETAOPEALLICOM2"));
                if (banques != null) {
...
//                //Recuperation des effets d'une Banque
    ...
    private void updateEtatOperations(DataBase db) throws SQLException {
        ...
        db.executeUpdate(l_sql);
        if (remiseHasCheques) {
            l_sql = "UPDATE CHEQUES SET DATECOMPENSATION='" + Utility.convertDateToString(Utility.convertStringToDate(Utility.getParam("DATECOMPENS_SRG"), "yyyyMMdd"), ResLoader.getMessages("patternDate")) + "',ETAT=" + Utility.getParam("CETAOPEALLICOM2ENV") + " WHERE ETAT =" + Utility.getParam("CETAOPEALLPREICOM2");
            db.executeUpdate(l_sql);
        }

        if (remiseHasEffets) {
            l_sql = "UPDATE EFFETS SET DATECOMPENSATION='" + Utility.convertDateToString(Utility.convertStringToDate(Utility.getParam("DATECOMPENS_SRG"), "yyyyMMdd"), ResLoader.getMessages("patternDate")) + "',ETAT=" + Utility.getParam("CETAOPEALLICOM2ENV") + " WHERE ETAT =" + Utility.getParam("CETAOPEALLPREICOM2");
            db.executeUpdate(l_sql);
        }
