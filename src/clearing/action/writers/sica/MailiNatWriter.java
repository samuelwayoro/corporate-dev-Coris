 ...
 * @author Patrick
 */
public class MailiNatWriter extends BinFileWriter {
        ...
        if (banques != null) {
            for (int i = 0; i < banques.length; i++) {
                ...
                if (Utility.getParam("GENMAILIPARBANQUE") != null && Utility.getParam("GENMAILIPARBANQUE").equalsIgnoreCase("1")) {
                    ...
                    //Recuperation des cheques d'une Banque
                    sql = "SELECT * FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAOPEALLICOM1ACC") + "," + Utility.getParam("CETAOPEALLICOM1ACCENVSIB") + ") AND TYPE_CHEQUE ='035' AND BANQUE LIKE '" + banque.getCodebanque() + "' AND length(trim(FICHIERIMAGE))>0   AND ETATIMAGE = " + Utility.getParam("CETAIMASTO");
                    ...
                    if (agences != null) {
                        for (int j = 0; j < agences.length; j++) {
                            ...
                            //Recuperation des cheques d'une Banque
                            sql = "SELECT * FROM CHEQUES WHERE  ETAT IN (" + Utility.getParam("CETAOPEALLICOM1ACC") + "," + Utility.getParam("CETAOPEALLICOM1ACCENVSIB") + ") AND TYPE_CHEQUE ='035' AND BANQUE LIKE '" + banque.getCodebanque() + "' AND AGENCE LIKE '" + agence.getCodeagence() + "' AND length(trim(FICHIERIMAGE))>0  AND ETATIMAGE = " + Utility.getParam("CETAIMASTO");
                            cheques35 = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
                            prepareCheques(cheques35);
        ...
        if (effets != null && effets.length > 0) {
           ...
           // enteteRemise.setTypeRemise("IMAGF");
            ...
            for (int j = 0; j < effets.length; j++) {
                ...
                effet.setFichiermaili(fileName);
