...
public class ChequeAllerSurCaisseDebitFlexCubeSQLWriter extends FlatFileWriter {
            ...
            for (int i = 0; i < cheques.length; i++) {
                if (!isValidLine(cheques[i])) {
                    ...
                    numLigne = createLinesSQL(current_cycle, current_period, numeroBatch, numLigne, "~", numeroBatch + "" + cheque.getIdcheque(), cheque.getMontantcheque(),
                            ...
                            cheque, "Paiement CHQ N°:" + Utility.bourrageGauche(Utility.trimLeadingZero(cheque.getNumerocheque()), 7, " ") + " du Bordereau N°" + cheque.getRefremise() + " "
                            + " Faveur " + cheque.getNombeneficiaire(), new File(fileName).getName()); //Utility.trimLeadingZero(cheque.getNumerocheque()), 7, " "
    ...
    private void createEnteteSQL() {
        ...
        line.append("LOAD DATA\n"
                ...
                + " CURR_NO,AMOUNT,DR_CR,UPLOAD_STAT,CCY_CD,INITIATION_DATE DATE \"" + Utility.getParam("DATE_FORMAT") + "\"  ,LCY_EQUIVALENT,EXCH_RATE,rel_cust,external_ref_no,related_account,INSTRUMENT_NO,TXT_FILE_NAME)\n"
                + "BEGINDATA");
        writeln(line.toString());
    }
...
//                + " CURR_NO,AMOUNT,DR_CR,UPLOAD_STAT,CCY_CD,INITIATION_DATE,LCY_EQUIVALENT,EXCH_RATE,rel_cust,external_ref_no,related_account,INSTRUMENT_NO)\n"
        ...
        line.append(referenceRelative); //external_ref_no
        line.append(separateur);
        line.append(compteFlexCube != null ? compteFlexCube.getNumcptex() : "" + aCheque.getNumerocompte()); // Utility.getParam("CPTATTCHQCAIFLEX")
        line.append(separateur);
        line.append(aCheque.getNumerocheque()); // Instrument No
        line.append(separateur);
        line.append(fileName); //  Filename
