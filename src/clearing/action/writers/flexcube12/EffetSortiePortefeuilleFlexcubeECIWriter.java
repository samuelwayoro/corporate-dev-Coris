 ...
 * @author hp
 */
public class EffetSortiePortefeuilleFlexcubeECIWriter extends FlatFileWriter {


    public EffetSortiePortefeuilleFlexcubeECIWriter() {
        setDescription("Envoi du fichier  vers le SIB");
    }
...
//        String fileName = Utility.getParam("SIB_IN_FOLDER") + File.separator + Utility.getParam("CPTENV_FILE_ROOTNAME") + compteur + Utility.getParam("SIB_FILE_EXTENSION");
        ...
        String fileName = Utility.getParam("SIB_IN_FOLDER") + File.separator + Utility.getParam("EFTSORTIE_FILE_ROOTNAME") + dateTraitement + "_" + numeroBatch + Utility.getParam("SIB_FILE_SQL_EXTENSION");
        ...
        //Population des effets en portefeuille dans WebClearing
        sql = "SELECT * FROM EFFETS WHERE ETAT IN (" + Utility.getParam("CETAOPEVALDELTA") + ")AND DATE_ECHEANCE = '" + dateCompensation + "'  ORDER BY REMISE";
        ...
        if (effets != null && 0 < effets.length) {
            ...
            //pour avoir le montant globale des effets etat=40
            //sql = "SELECT * FROM EFFETS WHERE ETAT IN (" + Utility.getParam("CETAOPEVALDELTA") + ") ORDER BY REMISE";
            sql = "SELECT * FROM EFFETS WHERE ETAT IN (" + Utility.getParam("CETAOPEVALDELTA") + ")AND DATE_ECHEANCE = '" + dateCompensation + "'  ORDER BY REMISE";
            ...
            for (int i = 0; i < effetscours.length; i++) {
                ...
                montantTotal += Long.parseLong(effet.getMontant_Effet());
                   ...
                   detbUploadDetail.setAMOUNT(new BigDecimal(effet.getMontant_Effet())); 
                   detbUploadDetail.setADDL_TEXT( Utility.getParam("LIBATTEFFETENTREFLEX2"));//Réception de Chèques
                   detbUploadDetail.setREL_CUST("");
                   // detbUploadDetail.setEXTERNAL_REF_NO("");
                   detbUploadDetail.setEXTERNAL_REF_NO(effet.getIdeffet().toString());
                   detbUploadDetail.setRELATED_ACCOUNT(Utility.getParam("CPTATTEFFETENTREFLEX2"));
                   ...
                   detbUploadDetail.setSOURCE_CODE(Utility.getParam("BATCH_TYPE"));
                   detbUploadDetail.setACCOUNT_BRANCH(Utility.getParam("FLEXMAINBRANCH"));//agence du compte
                   detbUploadDetail.setACCOUNT(Utility.getParam("CPTATTEFFETENTREFLEX1"));//REM ACCOUNT
                   detbUploadDetail.setDR_CR("C");//DR_CR Sens D ou C
                   detbUploadDetail.setTXN_CODE("F57");//Code transaction TXN_CODE
                   //detbUploadDetail.setAMOUNT(new BigDecimal(montantTotal)); //AMOUNT
                   detbUploadDetail.setAMOUNT(new BigDecimal(effet.getMontant_Effet())); 
                   detbUploadDetail.setADDL_TEXT( Utility.getParam("LIBATTEFFETENTREFLEX1"));
                   detbUploadDetail.setREL_CUST("");
                   //detbUploadDetail.setEXTERNAL_REF_NO("");
                   detbUploadDetail.setEXTERNAL_REF_NO(effet.getIdeffet().toString());
                   detbUploadDetail.setRELATED_ACCOUNT(Utility.getParam("CPTATTEFFETENTREFLEX1"));
                   detbUploadDetail.setTXT_FILE_NAME(new File(fileName).getName());
                   detbUploadDetails.add(detbUploadDetail);
                   
                   //fin sortie portefeuille 
                   //recouvrement debut 
                   ...
                   detbUploadDetail.setSOURCE_CODE(Utility.getParam("BATCH_TYPE"));
                   detbUploadDetail.setACCOUNT_BRANCH(Utility.getParam("FLEXMAINBRANCH"));//agence du compte
                   detbUploadDetail.setACCOUNT(Utility.getParam("CPTATTEFFETRECOUVFLEX2"));//REM ACCOUNT
                   detbUploadDetail.setDR_CR("D");//DR_CR Sens D ou C
                   detbUploadDetail.setTXN_CODE("F03");//Code transaction TXN_CODE
                   //detbUploadDetail.setAMOUNT(new BigDecimal(montantTotal)); //AMOUNT
                   detbUploadDetail.setAMOUNT(new BigDecimal(effet.getMontant_Effet())); 
                   detbUploadDetail.setADDL_TEXT( Utility.getParam("LIBATTEFFETRECOUVFLEX2"));//Réception de Chèques
                   detbUploadDetail.setREL_CUST("");
                   // detbUploadDetail.setEXTERNAL_REF_NO("");
                   detbUploadDetail.setEXTERNAL_REF_NO(effet.getIdeffet().toString());
                   detbUploadDetail.setRELATED_ACCOUNT(Utility.getParam("CPTATTEFFETRECOUVFLEX2"));
                   detbUploadDetail.setTXT_FILE_NAME(new File(fileName).getName());
                   detbUploadDetails.add(detbUploadDetail);
                   ...
                   detbUploadDetail.setSOURCE_CODE(Utility.getParam("BATCH_TYPE"));
                   detbUploadDetail.setACCOUNT_BRANCH(Utility.getParam("FLEXMAINBRANCH"));//agence du compte
                   detbUploadDetail.setACCOUNT(Utility.getParam("CPTATTEFFETRECOUVFLEX1"));//REM ACCOUNT
                   detbUploadDetail.setDR_CR("C");//DR_CR Sens D ou C
                   detbUploadDetail.setTXN_CODE("F57");//Code transaction TXN_CODE
                   //detbUploadDetail.setAMOUNT(new BigDecimal(montantTotal)); //AMOUNT
                   detbUploadDetail.setAMOUNT(new BigDecimal(effet.getMontant_Effet())); 
                   detbUploadDetail.setADDL_TEXT( Utility.getParam("LIBATTEFFETRECOUVFLEX1"));
                   detbUploadDetail.setREL_CUST("");
                   //detbUploadDetail.setEXTERNAL_REF_NO("");
                   detbUploadDetail.setEXTERNAL_REF_NO(effet.getIdeffet().toString());
                   detbUploadDetail.setRELATED_ACCOUNT(Utility.getParam("CPTATTEFFETRECOUVFLEX1"));
                   detbUploadDetail.setTXT_FILE_NAME(new File(fileName).getName());
                   detbUploadDetails.add(detbUploadDetail);
            ...
            //mise a jour des effets etat 50
            db.executeUpdate("UPDATE EFFETS SET ETAT =" + Utility.getParam("CETAOPEALLICOM1") + " WHERE ETAT=" +Utility.getParam("CETAOPEVALDELTA")+"AND DATE_ECHEANCE = '" + dateCompensation + "'");
