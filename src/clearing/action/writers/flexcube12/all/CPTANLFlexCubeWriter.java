 ...
 * @author Patrick
 */
public class CPTANLFlexCubeWriter extends FlatFileWriter {

    private String sep = "~";

    public CPTANLFlexCubeWriter() {
        setDescription("Envoi du fichier CPTANLn vers le SIB");
    }
    ...
    public void execute() throws Exception {
        ...
        System.out.println("Date Valeur DATE_FORMAT = " + dateValeur);
...
//        String fileName = Utility.getParam("SIB_IN_FOLDER") + File.separator + Utility.getParam("CPTANL_FILE_ROOTNAME") + compteur + Utility.getParam("SIB_FILE_EXTENSION");
        ...
        String fileName = Utility.getParam("SIB_IN_FOLDER") + File.separator + Utility.getParam("CPTANL_FILE_ROOTNAME") + dateTraitement + "_" + numeroBatch + Utility.getParam("SIB_FILE_SQL_EXTENSION");
...
// Population
        sql = "SELECT * FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAOPEALLICOM1ACCENVSIB") + "," + Utility.getParam("CETAOPEREJRET") + "," + Utility.getParam("CETAOPEREJRETENVSIB") + ") AND DATECOMPENSATION='" + dateCompensation + "' ORDER BY REMISE";
        ...
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i += j) {
                ...
                sql = "SELECT * FROM CHEQUES WHERE REMISE=" + cheques[i].getRemise() + " AND ETAT IN (" + Utility.getParam("CETAOPEALLICOM1ACCENVSIB") + "," + Utility.getParam("CETAOPEREJRET") + "," + Utility.getParam("CETAOPEREJRETENVSIB") + ") AND DATECOMPENSATION='" + dateCompensation + "' ORDER BY REMISE";
            ...
            setDescription(getDescription() + " exécuté avec succès:<br> Montant Total= " + Utility.formatNumber("" + montantTotal) + " - Nom de Fichier = " + fileName+ "<br>");
    ...
    private void createEnteteSQL() {
        ...
        line.append("LOAD DATA\n"
                ...
                + " CURR_NO,AMOUNT,DR_CR,UPLOAD_STAT,CCY_CD,INITIATION_DATE,LCY_EQUIVALENT DATE \"" + Utility.getParam("DATE_FORMAT") + "\",EXCH_RATE,rel_cust,external_ref_no,related_account,TXT_FILE_NAME,INSTRUMENT_NO)\n"
                + "BEGINDATA");
    ...
    private int writeFile(DETB_UPLOAD_DETAIL[] detbUploadDetailList) {
        ...
        for (DETB_UPLOAD_DETAIL detbUploadDetail : detbUploadDetailList) {
            ...
            line.append(sep(Utility.convertDateToString(detbUploadDetail.getVALUE_DATE(), "ddMMyyyy")));
