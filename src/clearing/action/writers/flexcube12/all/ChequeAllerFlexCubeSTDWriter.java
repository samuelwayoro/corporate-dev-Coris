 ...
 * @author Patrick
 */
public class ChequeAllerFlexCubeSTDWriter extends FlatFileWriter {

    public ChequeAllerFlexCubeSTDWriter() {
...
// Population
        ...
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i += j) {
                ...
                if (chequesVal != null && 0 < chequesVal.length) {
                    ...
                    for (int x = 0; x < chequesVal.length; x++) {
                        ...
                        detbUploadDetail.setADDL_TEXT(Utility.bourrageGauche(aCheque.getRemise() + "", 7, "0") + " " + Utility.bourrageDroite(Utility.getParam("LIBCHQALEFLEX1"), 18, " ") + Utility.bourrageGauche(aCheque.getIdcheque() + "", 8, "0"));
            ...
            //CPTENV
            // Population
            sql = "SELECT * FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAOPEALLICOM1ACC") + ")  ORDER BY REMISE";
            Cheques[] chequesEnv = (Cheques[]) db.retrieveRowAsObject(sql, new Cheques());
            if (chequesEnv != null && 0 < chequesEnv.length) {
                long montantTotalEnv = 0;

                for (int x = 0; x < chequesEnv.length; x++) {
                    montantTotalEnv += Long.parseLong(chequesEnv[x].getMontantcheque());
                }
                line = new StringBuffer();
                line.append(Utility.getParam("FLEXMAINBRANCH"));
                line.append(Utility.bourrageDroite(Utility.getParam("CPTATTCHQALEFLEX5"), 16, " "));
                line.append(createBlancs(11, " "));
                line.append("D");
                line.append(Utility.bourrageGauche("" + montantTotalEnv, 16, "0"));
                line.append("F03");
                line.append(CMPUtility.getDate());
                line.append(createBlancs(8, " "));
                line.append(Utility.getParam("LIBCHQALEFLEX2"));
                ...
                detbUploadDetail.setDR_CR("D");//DR_CR Sens D ou C
                detbUploadDetail.setTXN_CODE("F03");//Code transaction TXN_CODE
                detbUploadDetail.setAMOUNT(new BigDecimal(montantTotalEnv)); //AMOUNT
                detbUploadDetail.setADDL_TEXT(CMPUtility.getDate()+ Utility.getParam("LIBCHQALEFLEX2"));
                detbUploadDetail.setREL_CUST((Utility.getParam("CPTATTCHQALEFLEX5").length() == 16) ? Utility.getParam("CPTATTCHQALEFLEX5").substring(6, 14) : "999999992");
                detbUploadDetail.setEXTERNAL_REF_NO(detbUploadDetail.getBATCH_NO());
                detbUploadDetail.setRELATED_ACCOUNT(Utility.getParam("CPTATTCHQALEFLEX5"));
                ...
                setDescription("Envoi en recouvrement exécuté avec succès:\n Montant Total Envoye= " + Utility.formatNumber("" + montantTotalEnv) + " - Nom de Fichier = " + fileName);
                logEvent("INFO", " Montant Total Envoye= " + Utility.formatNumber("" + montantTotalEnv));
