    ...
    public void execute() throws Exception {
        ...
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i++) {
                ...
                line.append(Utility.bourrageDroite(CMPUtility.getNumCptEx(cheque.getCompteremettant(), cheque.getAgenceremettant(), "1"), 16, " "));
                ...
                detbUploadDetail.setREL_CUST((compteFlexCube != null && compteFlexCube.getNumcptex().trim().length() != 9) ? compteFlexCube.getNumcptex().substring(0, 9) : "999999992");//compteFlexCube != null ? compteFlexCube.getNumcptex().substring(0, 9) : ""
                ...
                if (cheque.getMotifrejet().equalsIgnoreCase("201") || cheque.getMotifrejet().equalsIgnoreCase("202")) {
                    ...
                    detbUploadDetail.setDR_CR("D");//DR_CR Sens D ou C
                    detbUploadDetail.setTXN_CODE("Q17");//Code transaction TXN_CODE
                    detbUploadDetail.setAMOUNT(new BigDecimal(Utility.bourrageGauche(Utility.getParamOfType("COMDEBREJCHQALEFLEX", "CODE_COMMISSION").trim(), 16, " ").trim())); //AMOUNT
                    ...
                    detbUploadDetail.setTXN_CODE("Q17");//Code transaction TXN_CODE
                    detbUploadDetail.setAMOUNT(new BigDecimal(Utility.bourrageGauche(Utility.getParamOfType("COMCREREJCHQALEFLEX1", "CODE_COMMISSION").trim(), 16, " ").trim())); //AMOUNT
                    ...
                    detbUploadDetail.setDR_CR("C");//DR_CR Sens D ou C
                    detbUploadDetail.setTXN_CODE("Q17");//Code transaction TXN_CODE
                    detbUploadDetail.setAMOUNT(new BigDecimal(Utility.bourrageGauche(Utility.getParamOfType("COMCREREJCHQALEFLEX2", "CODE_COMMISSION").trim(), 16, " ").trim())); //AMOUNT
...
// Gestion cpt globalisation
            ...
            detbUploadDetail.setACCOUNT_BRANCH(Utility.getParam("FLEXMAINBRANCH"));//agence du compte
            detbUploadDetail.setACCOUNT(Utility.getParam("CPTGLOCOMREJCHQALE1"));//REM ACCOUNT
            detbUploadDetail.setDR_CR("C");//DR_CR Sens D ou C
            detbUploadDetail.setTXN_CODE("Q17");//Code transaction TXN_CODE
            detbUploadDetail.setAMOUNT(new BigDecimal(montantTotal)); //AMOUNT
            detbUploadDetail.setADDL_TEXT(dateValeur + Utility.bourrageDroite(Utility.getParam("LIBREJCHQALEFLEX4"), 25, " "));
            detbUploadDetail.setREL_CUST((Utility.getParam("CPTGLOCOMREJCHQALE1").length() == 16) ? Utility.getParam("CPTGLOCOMREJCHQALE1").substring(6, 14) : "999999992");
            detbUploadDetail.setEXTERNAL_REF_NO(detbUploadDetail.getBATCH_NO());
            detbUploadDetail.setRELATED_ACCOUNT(Utility.getParam("LIBREJCHQALEFLEX5"));
            detbUploadDetail.setTXT_FILE_NAME(new File(fileName).getName());
            detbUploadDetails.add(detbUploadDetail);
            /**
