 ...
 * @author Patrick
 */
public class ChequeAllerRejeteFlexCubeEBJWriter extends FlatFileWriter {

    public ChequeAllerRejeteFlexCubeEBJWriter() {
    ...
    public void execute() throws Exception {
        ...
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i++) {
                ...
                line.append(Utility.bourrageDroite(nomEmetteur.trim() + " " + Utility.getParam("LIBREJCHQALEFLEX1") + " " + Utility.bourrageGauche(cheque.getNumerocheque() + "", 7, "0"), 197, " "));
                line.append("030");
                ...
                detbUploadDetail.setREL_CUST( (compteFlexCube != null && compteFlexCube.getNumcptex()!=null && compteFlexCube.getNumcptex().length()>=12 )  ? compteFlexCube.getNumcptex().substring(0, 9) : "999999996");
                ...
                if (cheque.getMotifrejet().equalsIgnoreCase("201") || cheque.getMotifrejet().equalsIgnoreCase("202")) {
                    ...
                    line = new StringBuffer();
                    line.append(CMPUtility.getNumCptExAgence(cheque.getCompteremettant(), cheque.getAgenceremettant()));
                    line.append(Utility.bourrageDroite(CMPUtility.getNumCptEx(cheque.getCompteremettant(), cheque.getAgenceremettant(), "0"), 16, " "));//Utility.bourrageGZero(CMPUtility.getNumCptEx(cheque.getCompteremettant(), cheque.getAgenceremettant()), 16)
                    line.append(createBlancs(4, " "));
                    line.append("D");
                    ...
                    detbUploadDetail.setACCOUNT_BRANCH(compteFlexCube != null ? compteFlexCube.getAdresse2().trim() : Utility.getParam("TXN_BRANCH").charAt(0)
                            + cheque.getAgenceremettant().substring(3));//agence du compte
                    detbUploadDetail.setACCOUNT(compteFlexCube != null ? compteFlexCube.getNumcptex() : "" + cheque.getCompteremettant());//REM ACCOUNT
                    detbUploadDetail.setDR_CR("D");//DR_CR Sens D ou C
                    detbUploadDetail.setTXN_CODE("Q17");//Code transaction TXN_CODE
                    ...
                    detbUploadDetail.setADDL_TEXT(CMPUtility.getDate() + Utility.bourrageGauche(cheque.getNumerocheque(), 8, " ") + Utility.bourrageGZero(cheque.getIdcheque().toPlainString(), 8) + cheque.getMotifrejet());
                    detbUploadDetail.setREL_CUST( (compteFlexCube != null && compteFlexCube.getNumcptex()!=null && compteFlexCube.getNumcptex().length()>=12 )  ? compteFlexCube.getNumcptex().substring(0, 9) : "999999996");
                    detbUploadDetail.setEXTERNAL_REF_NO(cheque.getIdcheque() + detbUploadDetail.getBATCH_NO());
                    detbUploadDetail.setRELATED_ACCOUNT(compteFlexCube != null ? compteFlexCube.getNumcptex() : "");
                    detbUploadDetail.setTXT_FILE_NAME(new File(fileName).getName());
            ...
            detbUploadDetail.setREL_CUST((Utility.getParam("CPTGLOCOMREJCHQALE").length() == 16) ? Utility.getParam("CPTGLOCOMREJCHQALE").substring(6, 14) : "999999996");
            detbUploadDetail.setEXTERNAL_REF_NO(detbUploadDetail.getBATCH_NO());
