 ...
 * @author Patrick
 */
public class ChequeRetourRejeteFlexCubeEBFWriter extends FlatFileWriter {

    public ChequeRetourRejeteFlexCubeEBFWriter() {
...
//        String fileName = Utility.getParam("SIB_IN_FOLDER") + File.separator + Utility.getParam("CHQ_REJ_RET_FILE_ROOTNAME") + compteur + Utility.getParam("SIB_FILE_EXTENSION");
        ...
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i++) {
                ...
                String labelRejet = " ";
                if (cheque.getMotifrejet() != null && !cheque.getMotifrejet().trim().isEmpty()) {
                    labelRejet = Utility.getParamLabel(cheque.getMotifrejet());
                }
                ...
                line.append(Utility.bourrageDroite(Utility.getParam("LIBREJCHQRETFLEX1") + cheque.getNumerocheque() + " " + cheque.getMotifrejet() + " " + labelRejet.trim(), 100, " "));
                /**
                 * ligne F12
                 */
                ...
                if (cheque.getMotifrejet().equalsIgnoreCase("201") || cheque.getMotifrejet().equalsIgnoreCase("202")) {
                    ...
                    detbUploadDetail.setADDL_TEXT(dateValeur + Utility.bourrageGauche(cheque.getNumerocheque(), 8, " ") + Utility.bourrageGZero(cheque.getIdcheque().toPlainString(), 8) + " " + Utility.bourrageDroite(Utility.getParam("LIBREJCHQRETFLEX1"), 18, " ") + labelRejet.trim() + cheque.getMotifrejet());
                    detbUploadDetail.setREL_CUST(compteFlexCube != null ? compteFlexCube.getNumcptex().substring(0, 9) : "");
                    detbUploadDetail.setEXTERNAL_REF_NO(cheque.getIdcheque() + detbUploadDetail.getBATCH_NO());
                    detbUploadDetail.setRELATED_ACCOUNT(compteFlexCube != null ? compteFlexCube.getNumcptex() : "");
                    detbUploadDetail.setTXT_FILE_NAME(new File(fileName).getName());
                ...
                /**
                 * les frais de rejets ne sont applicables quâ€™aux motifs liés
                 * Ã  la provision. Par conséquent, les frais de 5.850 ne
                 * doivent pas Ãªtre prélevés cf mail du ven. 26/05/2017 17:01
                 */
...
// Gestion cpt globalisation
            ...
            line.append(dateValeur);
            line.append(createBlancs(8, " "));
            line.append(Utility.bourrageDroite(Utility.getParam("LIBREJCHQRETFLEX4").trim() + " " + Utility.convertDateToString(Utility.convertStringToDate(CMPUtility.getDate(), "yyyymmdd"), "dd/mm/yyyy"), 25, " "));
            line.append(createBlancs(14, " "));
            /**
             * * F12
             */
