 ...
 * @author Patrick
 */
public class ChequeAllerSurCaisseFlexCubeECIWriter extends FlatFileWriter {

    public ChequeAllerSurCaisseFlexCubeECIWriter() {
...
// Population
        sql = "SELECT * FROM CHEQUES WHERE ETAT=" + Utility.getParam("CETAOPESUPVALSURCAI") + " AND VILLEREMETTANT='02' ORDER BY REMISE"; //mise Ã  jour pour la pri
        ...
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i += j) {
                ...
                sql = "SELECT * FROM CHEQUES WHERE REMISE=" + cheques[i].getRemise() + "  AND VILLEREMETTANT='02'  AND ETAT =" + Utility.getParam("CETAOPESUPVALSURCAI");
...
//Creation ligne de chèque
                        ...
                        for (int x = 0; x < chequesVal.length; x++) {
                            ...
                            detbUploadDetail.setADDL_TEXT(dateValeur + Utility.bourrageGauche(aCheque.getNumerocheque(), 8, " ") + Utility.getParam("LIBCHQALEFLEX1") + aCheque.getNumerocheque() + aCheque.getIdcheque().toPlainString());
...
//Creation ligne de chèque
                        ...
                        for (int x = 0; x < chequesVal.length; x++) {
                            ...
                            detbUploadDetail.setACCOUNT_BRANCH(compteFlexCube != null ? compteFlexCube.getAdresse2().trim() : Utility.getParam("TXN_BRANCH").charAt(0) + aCheque.getAgence().substring(3));//agence du compte
                            detbUploadDetail.setACCOUNT(compteFlexCube != null ? compteFlexCube.getNumcptex() : "" + aCheque.getNumerocompte());//REM ACCOUNT
                            detbUploadDetail.setDR_CR("D");//DR_CR Sens D ou C
                            detbUploadDetail.setTXN_CODE("Q13");//Code transaction TXN_CODE
                            detbUploadDetail.setAMOUNT(new BigDecimal(aCheque.getMontantcheque())); //AMOUNT
                            detbUploadDetail.setADDL_TEXT(dateValeur + Utility.bourrageGauche(aCheque.getNumerocheque(), 8, " ")
                                    + Utility.getParam("LIBCHQALEFLEX1")
                                    + aCheque.getNumerocheque() + aCheque.getIdcheque().toPlainString());
...
//remove, la remise est deja a 65
                        //    db.executeUpdate("UPDATE REMISES SET ETAT=" + Utility.getParam("CETAREMENVSIB") + " WHERE IDREMISE=" + aCheque.getRemise());
                    }
