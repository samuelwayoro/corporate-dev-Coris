 ...
 * @author Patrick
 */
public class ChequeAllerNonCompFlexCubeEBJWriter extends FlatFileWriter {

    public ChequeAllerNonCompFlexCubeEBJWriter() {
...
//Creation ligne de chèque
                        ...
                        for (int x = 0; x < chequesVal.length; x++) {
                            ...
                            line.append(Utility.bourrageDroite(nomEmetteur.trim() + Utility.getParam("LIBCHQALEFLEX1") + " " + Utility.bourrageGauche(aCheque.getNumerocheque() + "", 7, "0"), 194, " "));
...
//Creation ligne de chèque
                        ...
                        for (int x = 0; x < chequesVal.length; x++) {
                            ...
                            Comptes compteFlexCubeBF = CMPUtility.getCompteESNFlexCube(aCheque.getCompteremettant(), aCheque.getAgenceremettant());
                            ...
                            detbUploadDetail.setACCOUNT_BRANCH(compteFlexCube != null ? compteFlexCube.getAdresse2().trim() : Utility.getParam("TXN_BRANCH").charAt(0) + aCheque.getAgence().substring(3));//agence du compte  cc
                            ...
                            detbUploadDetail.setTXN_CODE("Q13");//Code transaction TXN_CODE
                            detbUploadDetail.setAMOUNT(new BigDecimal(aCheque.getMontantcheque())); //AMOUNT
                            detbUploadDetail.setADDL_TEXT(dateValeurDebit + Utility.bourrageGauche(aCheque.getRemise() + "", 7, "0") + Utility.getParam("LIBCHQALEFLEX3") + " " + Utility.bourrageGauche(aCheque.getNumerocheque() + "", 7, "0"));

                            detbUploadDetail.setREL_CUST(compteFlexCubeBF != null ? compteFlexCubeBF.getNumcptex().substring(0, 9) : "");
                            detbUploadDetail.setEXTERNAL_REF_NO(aCheque.getIdcheque() + detbUploadDetail.getBATCH_NO());
                            detbUploadDetail.setRELATED_ACCOUNT(compteFlexCubeBF != null ? compteFlexCubeBF.getNumcptex() : "");
                            detbUploadDetail.setTXT_FILE_NAME(new File(fileName).getName());
