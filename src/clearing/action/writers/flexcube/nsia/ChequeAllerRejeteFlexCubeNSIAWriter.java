 ...
 * @author Patrick
 */
public class ChequeAllerRejeteFlexCubeNSIAWriter extends FlatFileWriter {

    public ChequeAllerRejeteFlexCubeNSIAWriter() {
    ...
    public void execute() throws Exception {
        ...
        sql = "SELECT * FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAOPEREJRET") + "," + Utility.getParam("CETAOPEREJRETENVSIB") + ") AND DATECOMPENSATION='" + dateCompensation + "'";
        ...
        if (cheques != null && 0 < cheques.length) {
            ...
            for (int i = 0; i < cheques.length; i++) {
                ...
                String nomEmetteur;
                if (cheque.getNomemetteur() == null || cheque.getNomemetteur().isEmpty()) {
                    nomEmetteur = " ";
                } else {
                    nomEmetteur = cheque.getNomemetteur();
                }
                ...
                if (cheque.getMotifrejet().equalsIgnoreCase("201") || cheque.getMotifrejet().equalsIgnoreCase("202")) {
                    ...
                    //detbUploadDetail.setADDL_TEXT(nomEmetteur.trim() + " " + Utility.bourrageDroite(Utility.getParam("LIBREJCHQALEFLEX1") + cheque.getNumerocheque() + " " + cheque.getMotifrejet() + " " + Utility.getParamLabel(cheque.getMotifrejet()).trim(), 100, " ") + Utility.bourrageGZero(cheque.getIdcheque().toPlainString(), 8) + cheque.getMotifrejet());
                    detbUploadDetail.setADDL_TEXT(Utility.getParam("LIBREJCHQALECPTFLEX") +" "+ Utility.getParamLabel(cheque.getMotifrejet()) + " CHQ " + CMPUtility.getLibelleBanque(cheque.getBanque())+ " No "+ Utility.bourrageGauche(cheque.getNumerocheque(), 8, " ")+ "  de " + nomEmetteur) ;
                    ...
                    detbUploadDetail.setPERIOD_CODE(current_period);
                    detbUploadDetail.setVALUE_DATE(dateValeur != null ? Utility.convertStringToDate(dateValeur, "yyyyMMdd") : new Date());//VALUE_DATE a recuperer par une requete
                    detbUploadDetail.setACCOUNT_BRANCH(compteFlexCube != null ? compteFlexCube.getAdresse2().trim() : Utility.getParam("TXN_BRANCH").charAt(0) + cheque.getAgenceremettant().substring(3));//agence du compte  cc
                    detbUploadDetail.setACCOUNT(compteFlexCube != null ? compteFlexCube.getNumcptex() : "" + cheque.getCompteremettant());//REM ACCOUNT
                    detbUploadDetail.setDR_CR("C");//DR_CR Sens D ou C
                    detbUploadDetail.setTXN_CODE("ACT");//Code transaction TXN_CODE
                    detbUploadDetail.setAMOUNT(new BigDecimal(cheque.getMontantcheque())); //AMOUNT
                    //detbUploadDetail.setADDL_TEXT( Utility.bourrageDroite(nomEmetteur.trim() + " " + Utility.getParam("LIBCHQALEFLEX1") + " " + CMPUtility.getLibelleBanque(cheque.getBanqueremettant()) + " No " + Utility.bourrageGauche(cheque.getNumerocheque() + "", 7, "0"), 100, " "));
                    detbUploadDetail.setADDL_TEXT(" CHQ " + CMPUtility.getLibelleBanque(cheque.getBanque())+ " No "+ Utility.bourrageGauche(cheque.getNumerocheque(), 8, " ")+ "  de " + nomEmetteur) ;
                    ...
                    detbUploadDetail.setADDL_TEXT(Utility.getParam("LIBREJCHQALEFRAISFLEX") +" "+ Utility.getParamLabel(cheque.getMotifrejet()) + " CHQ " + CMPUtility.getLibelleBanque(cheque.getBanque())+ " No "+ Utility.bourrageGauche(cheque.getNumerocheque(), 8, " ")+ "  de " + nomEmetteur) ;
                    detbUploadDetail.setREL_CUST("");
                    detbUploadDetail.setEXTERNAL_REF_NO("");
                    detbUploadDetail.setRELATED_ACCOUNT("");
                    detbUploadDetail.setTXT_FILE_NAME(new File(fileName).getName());
