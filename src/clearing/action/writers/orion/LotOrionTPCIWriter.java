 ...
 * @author Patrick
 */
public class LotOrionTPCIWriter extends FlatFileWriter {
    ...
    private void feedOperationWithCheque35(int j, Cheques cheque, Operation[] operations) {
        try {
            DataBase db = new DataBase(JDBCXmlReader.getDriver());
            db.open(JDBCXmlReader.getUrl(), JDBCXmlReader.getUser(), JDBCXmlReader.getPassword());
            Remises []remises = (Remises[]) db.retrieveRowAsObject("SELECT * FROM REMISES WHERE IDREMISE="+cheque.getRemise(), new Remises());
            String agenceDepot="01001";
            if(remises !=null && remises.length >0 ){
             agenceDepot = remises[0].getAgenceDepot();             
            }
            
            db.close();
            operations[j].setTypeOperation(cheque.getType_Cheque());
            operations[j].setRefOperation("" + cheque.getIdcheque());
            operations[j].setIdAgeRem(agenceDepot);
            ...
            operations[j].setCodeAval((cheque.getEscompte().intValue() == 0) ? "E" : "C");
            operations[j].setCodeCertif("0");
            operations[j].setAdrSouscripteur(createBlancs(100, " "));
            operations[j].setNomSouscripteur(Utility.bourrageDroite(cheque.getCodeutilisateur(), 4, " "));
            operations[j].setBlancs(createBlancs(286, " "));
        } catch (Exception ex) {
            Logger.getLogger(LotOrionTPCIWriter.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
