 ...
 * @author Patrick
 */
public class ChequeAllerNSIASitWriter extends FlatFileWriter {

    public ChequeAllerNSIASitWriter() {
                ...
                if((remises != null && 0 < remises.length) &&
                   (allChequesVal.length == remises[0].getNbOperation().intValue())){
                    ...
                    String line = Utility.bourrageGauche(aCheque.getRemise().toPlainString(), 12, "0")
                                                       ...
                                                       + createBlancs(7, "0")
                                                       + "00"+ aCheque.getBanqueremettant()
                                                       + CMPUtility.getAgenceACP(aCheque.getBanqueremettant(),aCheque.getAgenceremettant())
                                                       + Utility.computeCleRIB("00"+aCheque.getBanqueremettant(), CMPUtility.getAgenceACP(aCheque.getBanqueremettant(),aCheque.getAgenceremettant()),Utility.bourrageGauche(aCheque.getCompteremettant(), 12, "0"))//createBlancs(2, "0")
                    ...
                    for (int x = 0; x < chequesVal.length; x++) {
                        ...
                        line = Utility.bourrageGauche(aCheque.getRemise().toPlainString(), 12, "0")
                                                      ...
                                                      + "00"+aCheque.getBanque()
                                                      + CMPUtility.getAgenceACP(aCheque.getBanque(),aCheque.getAgence())
                                                    //  + aCheque.getRibcompte()
//                                                      + CMPUtility.getCodeBanqueDestinataire(aCheque.getBanque())
//                                                      + aCheque.getAgence()
                                                      + Utility.computeCleRIB("00"+aCheque.getBanque(), CMPUtility.getAgenceACP(aCheque.getBanque(),aCheque.getAgence()),Utility.bourrageGauche(aCheque.getNumerocompte(), 12, "0"))
