 ...
 * @author Patrick
 */
public class LotDeltaWriter extends FlatFileWriter {
  ...
  printLotVirement(virements, enteteLot);
    ...
    private void updateEtatOperations(DataBase db) throws SQLException {
        ...
        if (remiseHasCheques) {
            l_sql = "UPDATE CHEQUES SET ETAT="+ Utility.getParam("CETAOPEVALDELTA") + ", LOTSIB=1 WHERE  " + whereClause ;
            db.executeUpdate(l_sql);
        }

        if (remiseHasVirements) {
            l_sql = "UPDATE VIREMENTS SET ETAT="+ Utility.getParam("CETAOPEVALDELTA") + ", LOTSIB=1 WHERE  " + whereClause ;
            db.executeUpdate(l_sql);
        }

        if (remiseHasEffets) {
            l_sql = "UPDATE EFFETS SET SET ETAT="+ Utility.getParam("CETAOPEVALDELTA") + ", LOTSIB=1 WHERE  " + whereClause ;
            db.executeUpdate(l_sql);
        }
