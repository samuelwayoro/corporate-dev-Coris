 ...
 * @author Patrick
 */
public class LotDeltaOraBankCltLourdWriter extends FlatFileWriter {
    ...
    public LotDeltaOraBankCltLourdWriter() {
    ...
    public void execute() throws Exception {
        ...
        String whereClause = " ETAT=" + Utility.getParam("CETAOPEVALDELTA") + " AND LOTSIB IS NULL"; //+ " AND BANQUE NOT IN ('" + CMPUtility.getCodeBanque() + "','" + CMPUtility.getCodeBanqueSica3() + "') ";

        db.executeUpdate("UPDATE CHEQUES SET ETAT=" + Utility.getParam("CETAOPEVALDELTA") + " WHERE ETAT=" + Utility.getParam("CETAOPEVAL"));
              //  + " AND REMISE IN (SELECT IDREMISE FROM REMISES WHERE ETAT=" + Utility.getParam("CETAOPEVALDELTA") + ")");
       ...
       // db.executeUpdate("UPDATE REMISES SET ETAT=" + Utility.getParam("CETAOPEVALDELTA") + " WHERE ETAT=" + Utility.getParam("CETAOPEVAL")
       //        + " AND IDREMISE IN (SELECT REMISE FROM CHEQUES WHERE ETAT=" + Utility.getParam("CETAOPEVALDELTA") + ")");
       ...
       //        + " AND IDREMISE IN (SELECT REMISE FROM CHEQUES WHERE ETAT=" + Utility.getParam("CETAOPEVAL") + ")");
        ...
        //sql="UPDATE CHEQUES SET ETAT="+Utility.getParam("CETAOPEVAL2")+" WHERE GARDE='1' AND DATEECHEANCE=TO_CHAR(SYSDATE,'YYYY/MM/DD')";
        //db.executeUpdate(sql);
        ...
        if (refLot > 0) {
            setDescription(getDescription() + " Envoi de Lot d'opération aller vers le SIB");
        ...
        db.close();
        //Ajout de l'envoi des images pour ne pas avoir à créer de nouvelle tache
        ImageChequeDeltaWriter imageChequeDeltaWriter = new ImageChequeDeltaWriter();
        imageChequeDeltaWriter.execute();
    ...
    private void feedOperationWithCheque35(int j, Cheques cheque, Operation[] operations) {
        ...
        try {
            ...
            Logger.getLogger(LotDeltaOraBankCltLourdWriter.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        if (remises != null && remises.length > 0) {
            operations[j].setIdAgeRem(cheque.getBanqueremettant());
...
//            operations[j].setCodeAval((cheque.getEscompte().intValue() == 0) ? "E" : "C");
            ...
            operations[j].setCodeAval("C");
            operations[j].setCodeCertif("0");
            operations[j].setAdrSouscripteur(Utility.bourrageDroite("" + cheque.getRemise(), 50, " "));
