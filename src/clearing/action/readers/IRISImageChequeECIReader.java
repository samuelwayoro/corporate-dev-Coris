 ...
 * @author Patrick
 */
public class IRISImageChequeECIReader extends FlatFileReader {
    ...
    public IRISImageChequeECIReader() {
    ...
    public Comptes getInfoCompte(String numeroCompte) {
        try {
            ...
            if (numeroCompte.length() == 16) {
                Comptes[] comptes = (Comptes[]) db.retrieveRowAsObject("select * from comptes where numcptex like '%" + numeroCompte + "%'", new Comptes());
                ...
                if (comptes != null && comptes.length > 0) {
                    ...
                    comptes = (Comptes[]) db.retrieveRowAsObject("select * from comptes where numero like '%" + numeroCompte.subSequence(4, 16) + "%' AND AGENCE like '%" + numeroCompte.substring(0, 3) + "%'  ", new Comptes());
                    if (comptes != null && comptes.length > 0) {
                        comptes[0].setNom(comptes[0].getNom().replaceAll("\\p{Punct}", " "));
                        if (comptes[0].getPrenom() != null) {
                            comptes[0].setPrenom(comptes[0].getPrenom().replaceAll("\\p{Punct}", " "));
                ...
                if (line != null) {
                    if (line.startsWith("C")) {
                        ...
                        if (comptes != null) {
                            ...
                            aRemise.setCompteRemettant(comptes.getNumero());
                            getChamp(1);
                        ...
                        if (renameSucceeded == 3) {
                            ...
                            aRemise.setReference(""+lastIdRemise);
                            aRemise.setNomUtilisateur(repertoire.getId());
                            String sql = "SELECT * FROM UTILISATEURS WHERE LOGIN='" + repertoire.getId() + "'";
                            Utilisateurs[] utilisateurs = (Utilisateurs[]) db.retrieveRowAsObject(sql, new Utilisateurs());
                            if (utilisateurs != null && utilisateurs.length > 0) {
                                if (utilisateurs[0].getAdresse() != null) {
                                    aRemise.setAgenceDepot(utilisateurs[0].getAdresse().trim());
                                }
                            }
                        ...
                        getChamp(1);
                        aCheque.setSequence(new BigDecimal(getChamp(10)));
                        is.close();
                        ...
                        if (renameSucceeded == 3) {
                            ...
                            if (comptes != null) {
                                String sql = "SELECT * FROM REMISES WHERE COMPTEREMETTANT='" + comptes.getNumero() + "' AND MACHINESCAN='" + repertoire.getPartenaire() + "' AND ETAT IN (" + Utility.getParam("CETAREMERR") + "," + Utility.getParam("CETAREMSAIIRIS") + ") AND SEQUENCE=" + aCheque.getSequence() + " ORDER BY IDREMISE DESC";
                                ...
                                if (remises != null && remises.length > 0) {
                                    ...
                                    aCheque.setCompteremettant(comptes.getNumero());
                                    aCheque.setCoderepresentation(BigDecimal.ZERO);
                                ...
                                sql = "SELECT * FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAREMSAIIRIS") + "," + Utility.getParam("CETAOPEALLICOM1ACC") + "," + Utility.getParam("CETAOPEVAL") + "," + Utility.getParam("CETAOPEALLICOM1") + "," + Utility.getParam("CETAOPEALLICOM1ENV") + "," + Utility.getParam("CETAOPEALLPREICOM1") + "," + Utility.getParam("CETAOPEVALSURCAIENVSIB") + ") AND BANQUE='" + aCheque.getBanque() + "' AND AGENCE='" + aCheque.getAgence() + "' AND NUMEROCHEQUE='" + aCheque.getNumerocheque() + "' AND NUMEROCOMPTE='" + aCheque.getNumerocompte() + "'";
