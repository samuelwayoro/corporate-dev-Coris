 ...
 * @author Patrick
 */
public class WincorImageChequeReaderSICA3 extends FlatFileReader {
    ...
    public WincorImageChequeReaderSICA3() {
    }
    ...
    public File treatFile(File aFile, Repertoires repertoire) throws Exception {
        ...
        String[] remisesArray = wincorXmlReader.getRemises();

        if (nums != null) {
            wincorCheque = new WincorCheque[nums.length];

            for (int i = 0; i < wincorCheque.length; i++) {
                wincorCheque[i] = new WincorCheque(nums[i], cmc7s[i], cmc7oks[i], dateOs[i], agences[i], atms[i], compteRs[i], remisesArray[i], numBRemises[i], dteServers[i], images[i]);
            }
        }
        ...
        for (int i = 0; i < wincorCheque.length; i++) {
            ...
            if (versoFile.exists() && rectoFile.exists()) {
                ...
                if (renameSucceeded == 2) {
                    ...
                    String sql = "SELECT * FROM REMISES WHERE IDREMISE =" + lastIdRemise.toString();
