 ...
 * @author Patrick
 */
public class MailiMailoReader extends BinFileReader  {
    ...
    private String mailoExtFolder = Utility.getParam("MAILI_MAILO_FOLDER");
...
//MAJ ou INSERTION
            String sql = "SELECT * FROM CHEQUES_NUMERISCAN WHERE BANQUE ='" + aCheque.getBanque() +
            ...
            if (cheques != null && cheques.length > 0) {
                ...
                sql = "UPDATE CHEQUES_NUMERISCAN SET PATHIMAGE='" + imagePath + "',FICHIERIMAGE='" + imageName + "', ETAT=" + Utility.getParam("CETAOPERETREC") + " WHERE IDCHEQUE =" + cheques[0].getIdcheque();
                try {
                    db.executeUpdate(sql);
                    // db.updateRowByObjectByQuery(aCheque, "CHEQUES", sql);
                } catch (Exception ex) {
                    Logger.getLogger(MailiMailoReader.class.getName()).log(Level.SEVERE, null, ex);
                }
            // db.updateRowByObjectByQuery(aCheque, "CHEQUES", sql);

            } else {
                aCheque.setIdcheque(new BigDecimal(Utility.computeCompteur("IDCHEQUE", "CHEQUES_NUMERISCAN")));
                aCheque.setEtat(new BigDecimal(Utility.getParam("CETAOPERETIMA")));
                db.insertObjectAsRowByQuery(aCheque, "CHEQUES_NUMERISCAN");
            }
    ...
    private void readImageRecto(BufferedInputStream bis, int taille) throws Exception {
        ...
        if (MAXIMGSIZE > taille) {
            ...
            imageName = imageIndex.getNumCptTire() +
                    imageIndex.getCleRibDeb() +
                    ((imageIndex.getTypeOperation().equals("030")) ? imageIndex.getNumCheque() : imageIndex.getNumEffet()) +
