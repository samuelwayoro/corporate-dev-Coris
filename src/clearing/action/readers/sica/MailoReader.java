 ...
 * @author Patrick
 */
public class MailoReader extends BinFileReader {
...
//MAJ ou INSERTION
            String sql = "SELECT * FROM CHEQUES WHERE BANQUE ='" + aCheque.getBanque()
                    ...
                    + "' AND MONTANTCHEQUE ='" + aCheque.getMontantcheque() + "' AND ETAT IN (" + Utility.getParam("CETAOPERET")
                    + "," + Utility.getParam("CETAOPERETENVSIB") + ")";
            ...
            if (cheques != null && cheques.length > 0) {
                ...
                if (cheques[0].getEtat().equals(new BigDecimal(Integer.parseInt(Utility.getParam("CETAOPERET"))))) {
                    ...
                    sql = "UPDATE CHEQUES SET PATHIMAGE='" + imagePath + "',FICHIERIMAGE='" + imageName + "', ETAT=" + Utility.getParam("CETAOPERETRECENVSIB") + " WHERE IDCHEQUE =" + cheques[0].getIdcheque();
                    db.executeUpdate(sql);
                }
    ...
    public File treatFile(File aFile, Repertoires repertoire) {
        ...
        try {
            ...
            Logger.getLogger(Mailo3Reader.class.getName()).log(Level.INFO, null, ex);
            db.close();
        }
        return aFile;
    }
