 ...
 * @author Patrick
 */
public class SmartImageChequeSynReader extends FlatFileReader {
    ...
    public SmartImageChequeSynReader() {
        ...
        if (sequence != null) {
            ...
            if (!versoFileName.trim().isEmpty() && !dataFileName.trim().isEmpty() && !rectoFileName.trim().isEmpty()
                ...
                if (rExist || vExist || dExist) {
                    System.out.println("Fichier deja traite = " + rectoFileName + "|" + versoFileName + "|" + dataFileName);
                    ...
                    if (dataFile.length() == 0 || (Utility.getParam("STRONG_CHECK_CMC7") != null && Utility.getParam("STRONG_CHECK_CMC7").equals("1") && line.length() < Integer.parseInt(Utility.getParam("CMC7_MIN_LENGTH")))) {
                        ...
                        if (!waitFolder.exists()) {
                            if (!waitFolder.mkdirs()) {
                                logEvent("ERREUR", "Impossible de creer " + srvRemiseFilePath);
                        ...
                        if (rectoFile.exists() && new File(srvRemiseFilePath + File.separator + fileRoot + "f.jpg").exists()) {
                            rectoFile.delete();
                            System.out.println("Deplacement Recto Remise OK " + sequence);
                            renameSucceeded++;
                        } else {
                            System.out.println("Deplacement Recto Remise NON OK " + sequence);
                            logEvent("ERREUR", "Deplacement Recto Remise NON OK " + sequence + "| vers " + srvRemiseFilePath + File.separator + fileRoot + "f.jpg");
                        }
                        ...
                        if (versoFile.exists() && new File(srvRemiseFilePath + File.separator + fileRoot + "r.jpg").exists()) {
                            versoFile.delete();
                            System.out.println("Deplacement Verso Remise OK " + sequence);
                            renameSucceeded++;
                        } else {
                            System.out.println("Deplacement Verso Remise NON OK " + sequence);
                            logEvent("ERREUR", "Deplacement Verso Remise NON OK " + sequence + "| vers " + srvRemiseFilePath + File.separator + fileRoot + "r.jpg");
                        }
                        if (dataFile.delete()) {
