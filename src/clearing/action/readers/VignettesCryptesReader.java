...
import org.patware.utils.Decryptage;
 ...
 * @author Patrick
 */
public class VignettesCryptesReader extends FlatFileReader {

    public VignettesCryptesReader() {
    ...
    public File treatFile(File aFile, Repertoires repertoire) throws Exception {
        ...
        int compteur = 0;
        BufferedReader is = new BufferedReader(new StringReader(Decryptage.readCryptedFile(aFile.getAbsolutePath())));
        
        while ((line = is.readLine()) != null) {
            compteur++;
            System.out.println("Ligne "+compteur+" = "+ line);
            if(line.length()>=630)
            if (compteur != 1 ) {
                setCurrentLine(line);
