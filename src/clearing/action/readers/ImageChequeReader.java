 ...
 * @author Patrick
 */
public class ImageChequeReader extends FlatFileReader {
    ...
    public File treatFile(File aFile, Repertoires repertoire) throws Exception {
        ...
        if (sequence != null) {
            ...
            if (!versoFileName.trim().isEmpty() && !dataFileName.trim().isEmpty()
                    && versoFile.exists() && dataFile.exists()) {
...
//                    }
                    ...
                    if (line.startsWith("+")) {
                        getChamp(1);
                        aCheque.setNumerocheque(getChamp(7).trim().replaceAll("[\\p{Punct}\\p{Alpha}]", "_"));
                    } else {
                        aCheque.setNumerocheque(getChamp(7).trim().replaceAll("[\\p{Punct}\\p{Alpha}]", "_"));
                    }
                    getChamp(2);
                    aCheque.setBanque(getAlphaNumericCodeBanque(Utility.bourrageGauche(getChamp(5).trim().replaceAll("[\\p{Punct}\\p{Alpha}]", "_"), 5, "_")));
                    aCheque.setAgence(getChamp(5).trim().replaceAll("[\\p{Punct}\\p{Alpha}]", "_"));
                    aCheque.setRibcompte(getChamp(2).trim().replaceAll("[\\p{Punct}\\p{Alpha}]", "_"));
                    getChamp(2);
                    aCheque.setNumerocompte(getChamp(12).trim().replaceAll("[\\p{Punct}\\p{Alpha}]", "_"));
                    is.close();
                    ...
                    if (renameSucceeded == 3) {
                        ...
                        if (aCheque.getBanque() != null && aCheque.getBanque().equals(Utility.getParam("CODE_BANQUE_SICA3"))) {
                            aCheque.setEtat(new BigDecimal(Utility.getParam("CETAOPEVALSURCAI")));
                        } else {
