 ...
 * @author Patrick
 */
public class OutgoLiteReader extends FlatFileReader {
...
//                        break;
        ...
        sql = " SELECT * FROM CHEQUES WHERE ETAT IN (" + Utility.getParam("CETAOPEALLICOM1ACC") + ") AND DATECOMPENSATION = (SELECT MAX(DATECOMPENSATION) FROM CHEQUES WHERE DATECOMPENSATION < '" + dateCompensation +"')";
        ...
        if (cheques != null && cheques.length > 0) {
            ...
            for (int j = 0; j < cheques.length; j++) {
                ...
                if (chequesLite != null && chequesLite.length > 0) {
                    ...
                    chequesLite[0].setEtat(new BigDecimal(Long.parseLong(Utility.getParam("CETAOPEALLICOM1ACCENVSIB"))));
                    chequesLite[0].setRemcom(cheques[j].getRemcom());
                    chequesLite[0].setDatecompensation(cheques[j].getDatecompensation());
                    chequesLite[0].setMotifrejet(cheques[j].getMotifrejet());
                    sql = "IDCHEQUE = " + chequesLite[0].getIdcheque();
