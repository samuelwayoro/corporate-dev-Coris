 ...
 * @author Patrick
 */
public class ChequeRetourEISSignReader extends FlatFileReader {
        ...
        if (Utility.bourrageGauche(currentSumAccount + "", 12, "0").equalsIgnoreCase(sumAccount) &&
            ...
            for (int j = 0; j < cheques.length; j++) {
                ...
                sql = "UPDATE CHEQUES SET ETAT = " + Utility.getParam("CETAOPEALLICOM2") +
                        ", MOTIFREJET='" + aCheque.getMotifrejet() + "', CODEUTILISATEUR='" + aCheque.getCodeutilisateur() + "' WHERE IDCHEQUE=" + aCheque.getIdcheque() + " AND ETAT IN (" + Utility.getParam("CETAOPERETRECENVSIBVER") +","+ Utility.getParam("CETAOPERETRECENVSIBERR") +","+ Utility.getParam("CETAOPERETRECENVSIB") + ")";

                db.executeUpdate(sql);
                }else{
                    sql = "UPDATE CHEQUES SET CODEUTILISATEUR='" + aCheque.getCodeutilisateur() + "' WHERE IDCHEQUE=" + aCheque.getIdcheque() + " AND ETAT IN (" + Utility.getParam("CETAOPERETRECENVSIBVER") +","+ Utility.getParam("CETAOPERETRECENVSIBERR") + ")";
